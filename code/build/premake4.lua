...
if _OPTIONS["split-platforms"] then
  ...
  postbuildcommands({
    "mkdir " .. copybase .. "/bin/x32 ; cp -f " .. copybase .. "/lib/dynamic/x32/liborx*.so " .. copybase .. "/bin/x32",
  })

  configuration({ "linux", "not *Core*", "x64" })
  postbuildcommands({
    "mkdir " .. copybase .. "/bin/x64 ; cp -f " .. copybase .. "/lib/dynamic/x64/liborx*.so " .. copybase .. "/bin/x64",
  })
