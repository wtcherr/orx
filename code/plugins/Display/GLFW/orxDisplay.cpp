...
/* Linux ARM/ARM64 platforms default to OpenGL ES */
     ...
     (defined(__orxARM__) || defined(__orxARM64__))) ||                        \
    defined(__orxWEB__)
...
#define STBI_REALLOC(p, newsz)                                                 \
  orxMemory_Reallocate(p, newsz, orxMEMORY_TYPE_TEMP)
...
#define STBTT_malloc(sz, u)                                                    \
  orxMemory_Allocate((orxU32)(sz), orxMEMORY_TYPE_TEMP)
...
#ifdef __orxMSVC__
#pragma message(                                                               \
    "!!WARNING!! This plugin will only work in non-embedded mode when linked against a *DYNAMIC* version of GLFW!")
#else /* __orxMSVC__ */
...
#define orxDISPLAY_KU32_STATIC_FLAG_SHADER                                     \
  0x00000004                                         /**< Shader support flag */
...
#define orxDISPLAY_KU32_STATIC_FLAG_BACKGROUND                                 \
  0x00000020 /**< Background flag */
#define orxDISPLAY_KU32_STATIC_FLAG_EXT_READY                                  \
  0x00000040 /**< Extensions ready flag */
#define orxDISPLAY_KU32_STATIC_FLAG_FRAMEBUFFER                                \
  0x00000080 /**< Framebuffer support flag */
#define orxDISPLAY_KU32_STATIC_FLAG_DEPTHBUFFER                                \
  0x00000100 /**< Depthbuffer support flag */
#define orxDISPLAY_KU32_STATIC_FLAG_NO_RESIZE                                  \
  0x00000200 /**< No resize flag                                               \
              */
#define orxDISPLAY_KU32_STATIC_FLAG_IGNORE_EVENT                               \
  0x00000400 /**< Ignore event flag */
#define orxDISPLAY_KU32_STATIC_FLAG_NO_DECORATION                              \
  0x00000800 /**< No decoration flag */
#define orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN                                 \
  0x00001000 /**< Full screen flag */
#define orxDISPLAY_KU32_STATIC_FLAG_CUSTOM_IBO                                 \
  0x00002000 /**< Custom IBO flag */
#define orxDISPLAY_KU32_STATIC_FLAG_DEBUG_OUTPUT                               \
  0x00004000 /**< Debug output support flag */
#define orxDISPLAY_KU32_STATIC_FLAG_HIDDEN 0x00008000 /**< Hidden flag */
#define orxDISPLAY_KU32_STATIC_FLAG_VSYNC_FIX                                  \
  0x10000000 /**< VSync fix flag                                               \
              */
#define orxDISPLAY_KU32_STATIC_FLAG_UPDATE_REQUEST                             \
  0x20000000 /**< Video mode update request flag */
...
#define orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING                                  \
  0x00000010 /**< Smoothing flag                                               \
              */
...
#define orxDISPLAY_KU32_VERTEX_BUFFER_SIZE                                     \
  (4 * 2048) /**< 2048 items batch capacity */
#define orxDISPLAY_KU32_INDEX_BUFFER_SIZE                                      \
  (6 * 2048) /**< 2048 items batch capacity */
...
#define orxDISPLAY_KV_DEFAULT_DECORATED_POSITION                               \
  orx2F(100.0f), orx2F(120.0f), orxFLOAT_0
...
#define glUNIFORM(EXT, LOCATION, ...)                                          \
  do {                                                                         \
    if ((LOCATION) >= 0) {                                                     \
      glUniform##EXT(LOCATION, ##__VA_ARGS__);                                 \
      glASSERT();                                                              \
    }                                                                          \
  } while (orxFALSE)
...
#define glUNIFORM_NO_ASSERT(EXT, LOCATION, ...)                                \
  do {                                                                         \
    if ((LOCATION) >= 0) {                                                     \
      glUniform##EXT(LOCATION, ##__VA_ARGS__);                                 \
      (void)glGetError();                                                      \
...
static struct {
  ...
  int iShape;
} sastStandardCursorList[] = {orxDISPLAY_DECLARE_CURSOR(ARROW),
...
typedef enum __orxDISPLAY_BASISU_FORMAT_t {
  orxDISPLAY_BASISU_FORMAT_ASTC =
      (orxU32)basist::transcoder_texture_format::cTFASTC_4x4_RGBA,
  orxDISPLAY_BASISU_FORMAT_BC7 =
      (orxU32)basist::transcoder_texture_format::cTFBC7_RGBA,
  orxDISPLAY_BASISU_FORMAT_UNCOMPRESSED =
      (orxU32)basist::transcoder_texture_format::cTFRGBA32,
...
orxDisplay_GLFW_SetBlendMode(orxDISPLAY_BLEND_MODE _eBlendMode);
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetDestinationBitmaps(
    orxBITMAP **_apstBitmapList, orxU32 _u32Number);
orxSTATUS orxFASTCALL
orxDisplay_GLFW_SetVideoMode(const orxDISPLAY_VIDEO_MODE *_pstVideoMode);

int orxDisplay_GetBasisUInfo(void *_pInput, unsigned int _uiInputSize,
                             orxDISPLAY_BASISU_FORMAT _eFormat,
                             unsigned int *_puiWidth, unsigned int *_puiHeight,
                             unsigned int *_puiSize) {
  ...
  if (_uiInputSize >= sizeof(basist::ktx2_header)) {
    /* Valid? */
    if ((memcmp(pstHeader, basist::g_ktx2_file_identifier,
      ...
      *_puiSize = basist::basis_get_bytes_per_block_or_pixel(
                      (basist::transcoder_texture_format)_eFormat) *
                  ((basist::basis_transcoder_format_is_uncompressed(
                       (basist::transcoder_texture_format)_eFormat))
                       ? pstHeader->m_pixel_width * pstHeader->m_pixel_height
                       : ((pstHeader->m_pixel_width + 3) >> 2) *
                             ((pstHeader->m_pixel_height + 3) >> 2));
...
orxDisplay_GLFW_RenderInhibitor(const orxEVENT *_pstEvent) {
  /* Render stop? */
  ...
  if (apstMonitors != NULL) {
    ...
    if (s32Monitor > 0) {
      ...
      if (s32Monitor <= iCount) {
        ...
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                       "Invalid monitor: index [%d] out of range, max is [%d]. "
                       "Reverting to current monitor.",
                       s32Monitor, iCount);
      }
    }
    /* Should hide? */
      ...
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN,
    ...
    if (pstResult == NULL) {
      ...
      if (orxFLAG_TEST(sstDisplay.u32Flags,
                       orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN)) {
      ...
      /* Not hidden? */
      if (!orxFLAG_TEST(sstDisplay.u32Flags,
                        orxDISPLAY_KU32_STATIC_FLAG_HIDDEN)) {
        /* Stores it */
...
static orxINLINE void orxDisplay_GLFW_UpdateDefaultMode() {
  ...
  if (pstMonitor != orxNULL) {
    ...
    if ((glfwGetError(NULL) == GLFW_NO_ERROR) && (pstDesktopMode != NULL)) {
      ...
      sstDisplay.u32DefaultDepth =
          (orxU32)(pstDesktopMode->redBits + pstDesktopMode->greenBits +
                   pstDesktopMode->blueBits);
      ...
      /* Updates default mode */
      sstDisplay.u32DefaultWidth =
          orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_WIDTH)
              ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_WIDTH)
              : orxDISPLAY_KU32_DEFAULT_WIDTH;
      sstDisplay.u32DefaultHeight =
          orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_HEIGHT)
              ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_HEIGHT)
              : orxDISPLAY_KU32_DEFAULT_HEIGHT;
      sstDisplay.u32DefaultDepth =
          orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_DEPTH)
              ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_DEPTH)
              : orxDISPLAY_KU32_DEFAULT_DEPTH;
      sstDisplay.u32DefaultRefreshRate =
          orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_REFRESH_RATE)
              ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_REFRESH_RATE)
              : orxDISPLAY_KU32_DEFAULT_REFRESH_RATE;
...
static void GLAPIENTRY orxDisplay_GLFW_MessageCallback(
    GLenum _eSource, GLenum _eType, GLuint _uID, GLenum _eSeverity,
    GLsizei _iLength, const GLchar *_zMessage, const void *_pContext) {
  /* Relevant type? */
...
static void orxDisplay_GLFW_ResizeCallback(GLFWwindow *_pstWindow, int _iWidth,
                                           int _iHeight) {
  /* Not ignoring event? */
  if (!orxFLAG_TEST(sstDisplay.u32Flags,
                    orxDISPLAY_KU32_STATIC_FLAG_IGNORE_EVENT)) {
    /* Valid? */
    if ((_iWidth > 0) && (_iHeight > 0)) {
      /* Inits video mode */
      orxMemory_Zero(&(sstDisplay.stRequestVideoMode),
                     sizeof(orxDISPLAY_VIDEO_MODE));
      ...
      sstDisplay.stRequestVideoMode.bFullScreen =
          orxFLAG_TEST(sstDisplay.u32Flags,
                       orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN)
              ? orxTRUE
              : orxFALSE;
      ...
      orxFLAG_SET(sstDisplay.u32Flags,
                  orxDISPLAY_KU32_STATIC_FLAG_UPDATE_REQUEST,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    }
  }
...
static void orxDisplay_GLFW_DropCallback(GLFWwindow *_pstWindow, int _iNumber,
                                         const char **_azPaths) {
  ...
  /* Sends event */
  orxEVENT_SEND(orxEVENT_TYPE_SYSTEM, orxSYSTEM_EVENT_DROP, orxNULL, orxNULL,
                &stPayload);
...
static void orxDisplay_GLFW_PosCallback(GLFWwindow *_pstWindow, int _iX,
                                        int _iY) {
      ...
      for (i = 0; i < iCount; i++) {
        ...
        if (glfwGetError(NULL) == GLFW_NO_ERROR) {
          ...
          if ((glfwGetError(NULL) == GLFW_NO_ERROR) && (pstMode != NULL)) {
            /* Inside? */
            if ((_iX >= iX) && (_iY >= iY) && (_iX - iX < pstMode->width) &&
...
static void orxDisplay_GLFW_ContentScaleCallback(GLFWwindow *_pstWindow,
                                                 float _fScaleX,
                                                 float _fScaleY) {
  /* Not ignoring event? */
  if (!orxFLAG_TEST(sstDisplay.u32Flags,
    ...
    /* Forces a position update to prevent window from going back to its
     * previous monitor */
...
orxDisplay_GLFW_Update(const orxCLOCK_INFO *_pstClockInfo, void *_pContext) {
  /* Profiles */
  ...
  if (sstDisplay.pstWindow != orxNULL) {
    /* Foreground? */
    if (glfwGetWindowAttrib(sstDisplay.pstWindow, GLFW_ICONIFIED) ==
        GLFW_FALSE) {
      /* Wasn't in the foreground before? */
      if (orxFLAG_TEST(sstDisplay.u32Flags,
                       orxDISPLAY_KU32_STATIC_FLAG_BACKGROUND)) {
        ...
        if (orxEvent_SendShort(orxEVENT_TYPE_SYSTEM,
                               orxSYSTEM_EVENT_FOREGROUND) !=
            orxSTATUS_FAILURE) {
          /* Removes render inhibitor */
          orxEvent_RemoveHandler(orxEVENT_TYPE_RENDER,
                                 orxDisplay_GLFW_RenderInhibitor);
        }

        /* Updates foreground status */
        orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
                    orxDISPLAY_KU32_STATIC_FLAG_BACKGROUND);
      }
    }
    /* Background */
      ...
      if (!orxFLAG_TEST(sstDisplay.u32Flags,
        ...
        if (orxEvent_SendShort(orxEVENT_TYPE_SYSTEM,
          ...
          orxEvent_AddHandler(orxEVENT_TYPE_RENDER,
                              orxDisplay_GLFW_RenderInhibitor);

          /* Is VSync on? */
          if (orxFLAG_TEST(sstDisplay.u32Flags,
                           orxDISPLAY_KU32_STATIC_FLAG_VSYNC)) {
            orxCLOCK *pstClock;
            ...
            if ((pstClock != orxNULL) &&
                (orxClock_GetInfo(pstClock)->fTickSize == orxFLOAT_0)) {
              /* Updates status */
              sstDisplay.bOverrideClockTickSize = orxTRUE;

              /* Sets its tick size to match the refresh rate */
              orxClock_SetTickSize(
                  pstClock, orxFLOAT_1 / orxU2F(sstDisplay.u32RefreshRate));
            }
          }
        }

        /* Updates background status */
        orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_BACKGROUND,
                    orxDISPLAY_KU32_STATIC_FLAG_NONE);
      }
    }
    ...
    if (glfwWindowShouldClose(sstDisplay.pstWindow) != GLFW_FALSE) {
      ...
      if (orxEvent_SendShort(orxEVENT_TYPE_SYSTEM, orxSYSTEM_EVENT_CLOSE) ==
          orxSTATUS_FAILURE) {
        /* Resets close status */
    ...
    else if (orxFLAG_TEST(sstDisplay.u32Flags,
                          orxDISPLAY_KU32_STATIC_FLAG_UPDATE_REQUEST)) {
      int iWindowX = 0, iWindowY = 0;
...
static orxDISPLAY_PROJ_MATRIX *orxDisplay_GLFW_OrthoProjMatrix(
  ...
  /* Valid? */
  if ((fDeltaX != orxFLOAT_0) && (fDeltaY != orxFLOAT_0) &&
      (fDeltaZ != orxFLOAT_0)) {
    /* Clears matrix */
...
static orxINLINE void orxDisplay_GLFW_BindBitmap(const orxBITMAP *_pstBitmap) {
  ...
  for (i = 0, s32BestCandidate = 0, dBestTime = orxDOUBLE_MAX;
       i < (orxS32)sstDisplay.iTextureUnitNumber; i++) {
    /* Found? */
...
static orxINLINE orxDISPLAY_MATRIX *
orxDisplay_GLFW_InitMatrix(orxDISPLAY_MATRIX *_pmMatrix,
                           const orxDISPLAY_TRANSFORM *_pstTransform,
                           const orxBITMAP *_pstBitmap) {
  orxFLOAT fCos, fSin, fSCosX, fSCosY, fSSinX, fSSinY, fTX, fTY, fRotation,
      fSrcX, fSrcY, fScaleX, fScaleY;

  /* Updates rotation */
  fRotation = _pstTransform->fRotation +
              orxU2F(_pstTransform->eOrientation) * orxMATH_KF_PI_BY_2;
...
#define orxDISPLAY_LOAD_EXTENSION_FUNCTION(TYPE, FN)                           \
  FN = (TYPE)glfwGetProcAddress(#FN);

  /* Not already initialized? */
  if (!orxFLAG_TEST(sstDisplay.u32Flags,
                    orxDISPLAY_KU32_STATIC_FLAG_EXT_READY)) {
    orxU32 i;

#ifdef __orxDISPLAY_OPENGL_ES__

    /* Updates status flags */
    orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_FRAMEBUFFER,
                orxDISPLAY_KU32_STATIC_FLAG_NONE);
    ...
    if (glfwExtensionSupported("GL_EXT_framebuffer_object") != GLFW_FALSE) {
      ...
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLGENFRAMEBUFFERSEXTPROC,
                                         glGenFramebuffersEXT);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLDELETEFRAMEBUFFERSEXTPROC,
                                         glDeleteFramebuffersEXT);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLBINDFRAMEBUFFEREXTPROC,
                                         glBindFramebufferEXT);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLCHECKFRAMEBUFFERSTATUSEXTPROC,
                                         glCheckFramebufferStatusEXT);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLFRAMEBUFFERTEXTURE2DEXTPROC,
      ...
      /* Updates status flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
                  orxDISPLAY_KU32_STATIC_FLAG_FRAMEBUFFER);
    }
    ...
    glASSERT();
    sstDisplay.iDrawBufferNumber = orxMIN(
        sstDisplay.iDrawBufferNumber, orxDISPLAY_KU32_MAX_TEXTURE_UNIT_NUMBER);
    ...
    if (glfwExtensionSupported("GL_ARB_draw_buffers") != GLFW_FALSE) {
      ...
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLDRAWBUFFERSARBPROC,
                                         glDrawBuffersARB);
      ...
      glGetIntegerv(GL_MAX_DRAW_BUFFERS, &sstDisplay.iDrawBufferNumber);
      glASSERT();
      sstDisplay.iDrawBufferNumber =
          orxMIN(sstDisplay.iDrawBufferNumber,
                 orxDISPLAY_KU32_MAX_TEXTURE_UNIT_NUMBER);
    } else {
    ...
    /* Updates status flags */
    orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO,
                orxDISPLAY_KU32_STATIC_FLAG_NONE);
    ...
    if (glfwExtensionSupported("GL_ARB_vertex_buffer_object") != GLFW_FALSE) {
      ...
      /* Loads frame buffer extension functions */
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLGENBUFFERSARBPROC,
                                         glGenBuffersARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLDELETEBUFFERSARBPROC,
                                         glDeleteBuffersARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLBINDBUFFERARBPROC,
                                         glBindBufferARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLBUFFERDATAARBPROC,
                                         glBufferDataARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLBUFFERSUBDATAARBPROC,
                                         glBufferSubDataARB);
      ...
      /* Updates status flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
                  orxDISPLAY_KU32_STATIC_FLAG_VBO);
    }
...

    {
      ...
      if ((orxString_ToFloat(
               (const orxSTRING)glGetString(GL_SHADING_LANGUAGE_VERSION),
               &fShaderVersion, orxNULL) != orxSTATUS_FAILURE) &&
          (fShaderVersion + orxMATH_KF_EPSILON >= orx2F(1.1f))) {
        /* Pushes config section */
        orxConfig_PushSection(orxDISPLAY_KZ_CONFIG_SECTION);
        ...
        if (orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_SHADER_VERSION) ==
          ...
          orxConfig_SetU32(
              ...
              orxF2U(orxMath_Round(orx2F(100.0f) * fShaderVersion)));
        }
      ...
      sstDisplay.iTextureUnitNumber =
          orxMIN(sstDisplay.iTextureUnitNumber,
                 orxDISPLAY_KU32_MAX_TEXTURE_UNIT_NUMBER);
      glASSERT();

      /* Updates status flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_SHADER,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    }
    ...
    if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO) &&
      ...
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLCREATEPROGRAMOBJECTARBPROC,
                                         glCreateProgramObjectARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLCREATESHADEROBJECTARBPROC,
                                         glCreateShaderObjectARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLDELETEOBJECTARBPROC,
                                         glDeleteObjectARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLSHADERSOURCEARBPROC,
                                         glShaderSourceARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLCOMPILESHADERARBPROC,
                                         glCompileShaderARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLATTACHOBJECTARBPROC,
                                         glAttachObjectARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLLINKPROGRAMARBPROC,
                                         glLinkProgramARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLGETOBJECTPARAMETERIVARBPROC,
                                         glGetObjectParameterivARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLGETINFOLOGARBPROC,
                                         glGetInfoLogARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLUSEPROGRAMOBJECTARBPROC,
                                         glUseProgramObjectARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLGETUNIFORMLOCATIONARBPROC,
                                         glGetUniformLocationARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLBINDATTRIBLOCATIONARBPROC,
                                         glBindAttribLocationARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLENABLEVERTEXATTRIBARRAYARBPROC,
                                         glEnableVertexAttribArrayARB);
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLVERTEXATTRIBPOINTERARBPROC,
      ...
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLUNIFORMMATRIX4FVARBPROC,
                                         glUniformMatrix4fvARB);

#ifndef __orxLINUX__

      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLACTIVETEXTUREARBPROC,
                                         glActiveTextureARB);
    ...
    if (glfwExtensionSupported("GL_ARB_debug_output") != GLFW_FALSE) {
      /* Loads it */
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLDEBUGMESSAGECALLBACKARBPROC,
                                         glDebugMessageCallback);

      /* Updates status flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_DEBUG_OUTPUT,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    } else
#endif /* __orxMAC__ */
    {
      /* Updates status flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
                  orxDISPLAY_KU32_STATIC_FLAG_DEBUG_OUTPUT);
    }
    ...
    /* Has BC7 support? */
    if (glfwExtensionSupported("GL_ARB_texture_compression_bptc") !=
        GLFW_FALSE) {
      /* Selects format */
...
#if !defined(__orxDISPLAY_OPENGL_ES__) && !defined(__orxLINUX__) &&            \
    !defined(__orxMAC__)
    ...
    if (sstDisplay.eBasisUFormat != orxDISPLAY_BASISU_FORMAT_UNCOMPRESSED) {
      ...
      orxDISPLAY_LOAD_EXTENSION_FUNCTION(PFNGLCOMPRESSEDTEXIMAGE2DPROC,
                                         glCompressedTexImage2D);
    }
#endif /* !__orxDISPLAY_OPENGL_ES__ && !__orxLINUX__ && !__orxMAC */

    /* Updates status flags */
    orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_EXT_READY,
                orxDISPLAY_KU32_STATIC_FLAG_NONE);
  }
  ...
  orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_TEXTURE_UNIT_NUMBER,
                   (orxU32)sstDisplay.iTextureUnitNumber);
  orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_DRAW_BUFFER_NUMBER,
                   (orxU32)sstDisplay.iDrawBufferNumber);
  orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_MAX_TEXTURE_SIZE,
                   (orxU32)sstDisplay.iMaxTextureSize);
...
static int orxDisplay_GLFW_ReadSTBICallback(void *_hResource, char *_pBuffer,
                                            int _iSize) {
  /* Reads data */
  return (int)orxResource_Read((orxHANDLE)_hResource, _iSize, (orxU8 *)_pBuffer,
                               orxNULL, orxNULL);
}

static void orxDisplay_GLFW_SkipSTBICallback(void *_hResource, int _iOffset) {
  /* Seeks offset */
  orxResource_Seek((orxHANDLE)_hResource, _iOffset,
...
static int orxDisplay_GLFW_EOFSTBICallback(void *_hResource) {
  /* End of buffer? */
  return (orxResource_Tell((orxHANDLE)_hResource) ==
          orxResource_GetSize(_hResource))
             ? 1
             : 0;
}

static orxSTATUS orxFASTCALL
orxDisplay_GLFW_DecompressBitmapCallback(void *_pContext) {
  ...
  if (sstDisplay.u32Flags & orxDISPLAY_KU32_STATIC_FLAG_READY) {
    /* Cursor? */
    if (orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
                     orxDISPLAY_KU32_BITMAP_FLAG_CURSOR)) {
      /* Successful? */
      if (pstInfo->pu8ImageBuffer != orxNULL) {
        ...
        /* Creates cursor */
        sstDisplay.pstCursor =
            glfwCreateCursor(&stImage, (int)(pstInfo->pstBitmap->fWidth),
                             (int)(pstInfo->pstBitmap->fHeight));
      ...
      /* Clears loading flag */
      orxFLAG_SET(pstInfo->pstBitmap->u32Flags,
                  orxDISPLAY_KU32_BITMAP_FLAG_NONE,
                  orxDISPLAY_KU32_BITMAP_FLAG_LOADING);

      /* Deletes bitmap */
      orxDisplay_DeleteBitmap(pstInfo->pstBitmap);
    }
    /* Icon? */
    else if (orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
                          orxDISPLAY_KU32_BITMAP_FLAG_ICON)) {
      /* Tracks video memory */
      orxMEMORY_TRACK(VIDEO, pstInfo->pstBitmap->u32DataSize, orxTRUE);
      ...
      if (pstInfo->pu8ImageBuffer != orxNULL) {
        /* Adds icon image */
        sstDisplay.astIconList[sstDisplay.s32IconNumber].width =
            (int)(pstInfo->uiWidth);
        sstDisplay.astIconList[sstDisplay.s32IconNumber].height =
            (int)(pstInfo->uiHeight);
        sstDisplay.astIconList[sstDisplay.s32IconNumber].pixels =
            (unsigned char *)pstInfo->pu8ImageBuffer;
      ...
      if (sstDisplay.s32PendingIconCount == 1) {
        ...
        if (sstDisplay.s32IconNumber > 0) {
          /* Sets window icons */
          glfwSetWindowIcon(sstDisplay.pstWindow, (int)sstDisplay.s32IconNumber,
                            sstDisplay.astIconList);
        }
    ...
    /* Texture */
      ...
      pstInfo->pstBitmap->fBorderFix =
          ((pstInfo->uiWidth > 2) && (pstInfo->uiHeight > 2))
      ...
      pstInfo->pstBitmap->fRecRealWidth =
          orxFLOAT_1 / orxU2F(pstInfo->pstBitmap->u32RealWidth);
      pstInfo->pstBitmap->fRecRealHeight =
      ...
      orxVector_Set(&(pstInfo->pstBitmap->stClip.vBR),
                    pstInfo->pstBitmap->fWidth, pstInfo->pstBitmap->fHeight,
                    orxFLOAT_0);
      ...
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                      orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
                                   orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING)
                          ? GL_LINEAR
                          : GL_NEAREST);
      glASSERT();
      ...
      if ((pstInfo->bIsBasisU != orxFALSE) &&
        ...
        default: {
          /* Logs message */
          orxASSERT(orxFALSE && "Invalid Basis Universal format [%u] for this "
                                "platform, this should *not* happen.",
                    sstDisplay.eBasisUFormat);
          break;
        }
        }

        /* Loads compressed data */
        glCompressedTexImage2D(GL_TEXTURE_2D, 0, eInternalFormat,
                               (GLsizei)pstInfo->pstBitmap->u32RealWidth,
                               (GLsizei)pstInfo->pstBitmap->u32RealHeight, 0,
                               (GLsizei)pstInfo->pstBitmap->u32DataSize,
                               pstInfo->pu8ImageBuffer);
      } else {
        /* Loads data */
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
                     (GLsizei)pstInfo->pstBitmap->u32RealWidth,
                     (GLsizei)pstInfo->pstBitmap->u32RealHeight, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE,
      ...
      glBindTexture(
          GL_TEXTURE_2D,
          (sstDisplay.apstBoundBitmapList[sstDisplay.s32ActiveTextureUnit] !=
           orxNULL)
              ? sstDisplay.apstBoundBitmapList[sstDisplay.s32ActiveTextureUnit]
                    ->uiTexture
              : 0);
      ...
      if (orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
        ...
        if (pstInfo->pu8ImageBuffer == orxNULL) {
          /* Logs message */
          orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                         "Couldn't process data for bitmap <%s>: an empty "
                         "texture will be used instead.",
                         pstInfo->pstBitmap->zLocation);
        }
      }
      ...
      stPayload.stBitmap.u32ID = (pstInfo->pu8ImageBuffer != orxNULL)
                                     ? (orxU32)pstInfo->pstBitmap->uiTexture
                                     : orxU32_UNDEFINED;
      ...
      /* Sends event */
      orxEVENT_SEND(orxEVENT_TYPE_DISPLAY, orxDISPLAY_EVENT_LOAD_BITMAP,
                    pstInfo->pstBitmap, orxNULL, &stPayload);

      /* Asked for deletion? */
      if (orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
                       orxDISPLAY_KU32_BITMAP_FLAG_DELETE)) {
        /* Deletes it */
        orxDisplay_DeleteBitmap(pstInfo->pstBitmap);
      }
    }
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_DecompressBitmap(void *_pContext) {
  ...
  if (sstDisplay.u32Flags & orxDISPLAY_KU32_STATIC_FLAG_READY) {
    ...
    eFormat = orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
                           orxDISPLAY_KU32_BITMAP_FLAG_CURSOR |
                               orxDISPLAY_KU32_BITMAP_FLAG_ICON)
                  ? orxDISPLAY_BASISU_FORMAT_UNCOMPRESSED
                  : sstDisplay.eBasisUFormat;
    ...
    if ((uiDataSize = orxDisplay_GetBasisUInfo(
             pstInfo->pu8ImageSource, (unsigned int)pstInfo->s64Size, eFormat,
             &(pstInfo->uiWidth), &(pstInfo->uiHeight),
             (unsigned int *)&(pstInfo->u32DataSize))) != 0) {
      /* Allocates image data */
      pu8ImageData =
          (orxU8 *)orxMemory_Allocate(uiDataSize, orxMEMORY_TYPE_TEMP);
      ...
      if (pu8ImageData != orxNULL) {
        ...
        if (sstDisplay.spoKTX2Transcoder) {
          /* Inits it */
          if (sstDisplay.spoKTX2Transcoder->init(
                  pstInfo->pu8ImageSource, (unsigned int)pstInfo->s64Size)) {
            basist::ktx2_image_level_info sstInfo;

            /* Retrieves image info */
            if (sstDisplay.spoKTX2Transcoder->get_image_level_info(sstInfo, 0,
                                                                   0, 0)) {
              unsigned int uiRealSize;

              /* Gets its real size */
              uiRealSize = basis_get_bytes_per_block_or_pixel(
                               (basist::transcoder_texture_format)eFormat) *
                           ((basis_transcoder_format_is_uncompressed(
                                (basist::transcoder_texture_format)eFormat))
                                ? sstInfo.m_orig_width * sstInfo.m_orig_height
                                : sstInfo.m_total_blocks);
              ...
              if (uiDataSize >= uiRealSize) {
                ...
                if (sstDisplay.spoKTX2Transcoder->start_transcoding()) {
                  /* Transcodes image */
                  if (sstDisplay.spoKTX2Transcoder->transcode_image_level(
                          0, 0, 0, pu8ImageData,
                          uiRealSize /
    ...
    /* Is QOI? */
      ...
      /* Decodes it */
      pu8ImageData = (unsigned char *)qoi_decode(
          pstInfo->pu8ImageSource, (int)pstInfo->s64Size, &stDesc, 4);
      ...
      /* Loads image */
      pu8ImageData = stbi_load_from_memory(
          (unsigned char *)pstInfo->pu8ImageSource, (int)pstInfo->s64Size,
          (int *)&(pstInfo->uiWidth), (int *)&(pstInfo->uiHeight),
          (int *)&uiBytesPerPixel, STBI_rgb_alpha);
    ...
    if (pu8ImageData != NULL) {
      ...
      /* Clears info */
      pstInfo->uiWidth = pstInfo->uiHeight = pstInfo->uiRealWidth =
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_ProcessFont(void *_pContext) {
  ...
  if (sstDisplay.u32Flags & orxDISPLAY_KU32_STATIC_FLAG_READY) {
    ...
    /* Gets buffer size */
    u32Size = orxF2U(pstLoadInfo->stLoadInfo.pstBitmap->fWidth *
                     pstLoadInfo->stLoadInfo.pstBitmap->fHeight);

    /* Allocates image buffers */
    pu8Buffer = (pstLoadInfo->bSDF != orxFALSE)
                    ? orxNULL
                    : (orxU8 *)orxMemory_Allocate(u32Size, orxMEMORY_TYPE_TEMP);
    pu8ImageData =
        (orxU8 *)orxMemory_Allocate(4 * u32Size, orxMEMORY_TYPE_TEMP);

    /* Valid? */
    if ((pu8ImageData != orxNULL) &&
        ((pstLoadInfo->bSDF != orxFALSE) || (pu8Buffer != orxNULL))) {
      ...
      /* For all glyphs */
      for (i = 0, s32X = orxF2S(pstLoadInfo->vCharacterSpacing.fX),
          s32Y = orxF2S(pstLoadInfo->vCharacterSpacing.fY),
          s32Count = (orxS32)pstLoadInfo->u32GlyphCount,
          s32TextureWidth = orxF2S(pstLoadInfo->stLoadInfo.pstBitmap->fWidth);
           i < s32Count; i++) {
        orxS32 s32Width;
        ...
        if (s32X + s32Width > s32TextureWidth) {
          ...
          s32Y += orxF2S(pstLoadInfo->vCharacterSize.fY +
                         pstLoadInfo->vCharacterSpacing.fY);
        }
        ...
        if (pstLoadInfo->bSDF != orxFALSE) {
          ...
          /* Gets its shape */
          s32VertexCount = stbtt_GetGlyphShape(
              &(pstLoadInfo->stFontInfo), pstLoadInfo->astGlyphList[i].s32Index,
              &astVertexList);
          ...
          if (s32VertexCount > 0) {
            ...
            for (j = 0; j < s32VertexCount; ++j) {
              ...
              case STBTT_vline: {
                msdfgen::Point2 stPrevious((double)(astVertexList[j - 1].x),
                                           (double)(astVertexList[j - 1].y));
                msdfgen::Point2 stCurrent((double)(astVertexList[j].x),
                                          (double)(astVertexList[j].y));
                stShape.contours.back().addEdge(
                    msdfgen::EdgeHolder(stPrevious, stCurrent));
                break;
              }
              case STBTT_vcurve: {
                msdfgen::Point2 stPrevious((double)(astVertexList[j - 1].x),
                                           (double)(astVertexList[j - 1].y));
                msdfgen::Point2 stC0((double)(astVertexList[j].cx),
                                     (double)(astVertexList[j].cy));
                msdfgen::Point2 stCurrent((double)(astVertexList[j].x),
                                          (double)(astVertexList[j].y));
                stShape.contours.back().addEdge(
                    msdfgen::EdgeHolder(stPrevious, stC0, stCurrent));
                break;
              }
              case STBTT_vcubic: {
                msdfgen::Point2 stPrevious((double)(astVertexList[j - 1].x),
                ...
                msdfgen::Point2 stC0((double)(astVertexList[j].cx),
                                     (double)(astVertexList[j].cy));
                msdfgen::Point2 stC1((double)(astVertexList[j].cx1),
                                     (double)(astVertexList[j].cy1));
                msdfgen::Point2 stCurrent((double)(astVertexList[j].x),
                                          (double)(astVertexList[j].y));
                stShape.contours.back().addEdge(
                    msdfgen::EdgeHolder(stPrevious, stC0, stC1, stCurrent));
                break;
              }
              }
            }
            ...
            msdfgen::Bitmap<float, 4> oBitmap(
                (int)pstLoadInfo->astGlyphList[i].stGlyph.fWidth,
                (int)pstLoadInfo->vCharacterSize.fY);

            /* Inits transformation */
            msdfgen::Vector2 vScale(pstLoadInfo->vFontScale.fX,
                                    pstLoadInfo->vFontScale.fY);
            msdfgen::Vector2 vOffset(pstLoadInfo->astGlyphList[i].stGlyph.fX /
                                         pstLoadInfo->vFontScale.fX,
                                     pstLoadInfo->astGlyphList[i].stGlyph.fY /
                                         pstLoadInfo->vFontScale.fY);
            msdfgen::SDFTransformation stTransformation(
                msdfgen::Projection(vScale, vOffset),
                msdfgen::Range(0.25f * pstLoadInfo->vCharacterSize.fY /
                               pstLoadInfo->vFontScale.fY));
            ...
            for (int y = 0; y < oBitmap.height(); y++) {
              for (int x = 0; x < oBitmap.width(); x++) {
                pu8ImageData[((s32X + x) + (s32Y + y) * s32TextureWidth) * 4 +
                             0] = msdfgen::pixelFloatToByte(oBitmap(x, y)[0]);
                pu8ImageData[((s32X + x) + (s32Y + y) * s32TextureWidth) * 4 +
                             1] = msdfgen::pixelFloatToByte(oBitmap(x, y)[1]);
                pu8ImageData[((s32X + x) + (s32Y + y) * s32TextureWidth) * 4 +
                             2] = msdfgen::pixelFloatToByte(oBitmap(x, y)[2]);
                pu8ImageData[((s32X + x) + (s32Y + y) * s32TextureWidth) * 4 +
                             3] = msdfgen::pixelFloatToByte(oBitmap(x, y)[3]);
              }
            }
          }
          ...
          stbtt_MakeGlyphBitmap(
              &(pstLoadInfo->stFontInfo),
              pu8Buffer + s32X +
                  orxF2S(pstLoadInfo->astGlyphList[i].stGlyph.fX) +
                  ((s32Y + orxF2S(pstLoadInfo->astGlyphList[i].stGlyph.fY)) *
                   s32TextureWidth),
              s32Width - orxF2S(pstLoadInfo->astGlyphList[i].stGlyph.fX),
              orxF2S(pstLoadInfo->vCharacterSize.fY -
                     pstLoadInfo->astGlyphList[i].stGlyph.fY),
              s32TextureWidth, pstLoadInfo->vFontScale.fX,
              pstLoadInfo->vFontScale.fY,
              pstLoadInfo->astGlyphList[i].s32Index);
        }
      ...
      pstLoadInfo->stLoadInfo.uiWidth =
          orxF2U(pstLoadInfo->stLoadInfo.pstBitmap->fWidth);
      pstLoadInfo->stLoadInfo.uiHeight =
          orxF2U(pstLoadInfo->stLoadInfo.pstBitmap->fHeight);
      pstLoadInfo->stLoadInfo.u32DataSize = 4 *
                                            pstLoadInfo->stLoadInfo.uiWidth *
                                            pstLoadInfo->stLoadInfo.uiHeight;
      ...
      if (pstLoadInfo->bSDF == orxFALSE) {
        /* For all pixels */
        for (i = 0; i < (orxS32)u32Size; i++) {
          /* Sets it as white pixel with varying opacity */
          pu8ImageData[i * 4 + 0] = pu8ImageData[i * 4 + 1] =
      ...
      /* Clears info */
      pstLoadInfo->stLoadInfo.uiWidth = pstLoadInfo->stLoadInfo.uiHeight =
...
static void orxFASTCALL orxDisplay_GLFW_ReadResourceCallback(
    orxHANDLE _hResource, orxS64 _s64Size, void *_pBuffer, void *_pContext) {
  orxDISPLAY_LOAD_INFO *pstInfo;

  /* Allocates load info */
  pstInfo = (orxDISPLAY_LOAD_INFO *)orxMemory_Allocate(
      sizeof(orxDISPLAY_LOAD_INFO), orxMEMORY_TYPE_TEMP);
  ...
  if (orxFLAG_TEST(pstInfo->pstBitmap->u32Flags,
    ...
    if (orxThread_RunTask(&orxDisplay_GLFW_DecompressBitmap,
                          orxDisplay_GLFW_DecompressBitmapCallback, orxNULL,
                          (void *)pstInfo) == orxSTATUS_FAILURE) {
      /* Frees load info */
      orxMemory_Free(pstInfo);
    }
  } else {
...
static void orxDisplay_GLFW_WriteResourceCallback(void *_pContext, void *_pData,
                                                  int _iSize) {
  /* Writes resource synchronously */
  orxResource_Write((orxHANDLE)_pContext, (orxS64)_iSize, _pData, orxNULL,
                    orxNULL);
}
...
static orxSTATUS orxFASTCALL orxDisplay_GLFW_SaveBitmapData(void *_pContext) {
  ...
  /* Gets extension */
  zExtension =
      orxString_GetExtension(orxResource_GetLocation(pstInfo->hResource));

  /* PNG? */
  if (orxString_ICompare(zExtension, "png") == 0) {
    /* Saves image to disk */
    eResult = stbi_write_png_to_func(&orxDisplay_GLFW_WriteResourceCallback,
                                     pstInfo->hResource, pstInfo->u32Width,
                                     pstInfo->u32Height, 4,
                                     pstInfo->pu8ImageData, 0) != 0
                  ? orxSTATUS_SUCCESS
                  : orxSTATUS_FAILURE;
  }
  /* QOI? */
    ...
    if (pBuffer != NULL) {
      /* Saves image to disk */
      if (orxResource_Write(pstInfo->hResource, (orxS64)iSize, pBuffer, orxNULL,
                            orxNULL) == (orxS64)iSize) {
        /* Updates result */
        eResult = orxSTATUS_SUCCESS;
      }
  ...
  /* JPG? */
           ...
           (orxString_ICompare(zExtension, "jpeg") == 0)) {
    /* Saves image to disk */
    eResult = stbi_write_jpg_to_func(&orxDisplay_GLFW_WriteResourceCallback,
                                     pstInfo->hResource, pstInfo->u32Width,
                                     pstInfo->u32Height, 4,
                                     pstInfo->pu8ImageData, 0) != 0
                  ? orxSTATUS_SUCCESS
  ...
  /* BMP? */
    ...
    /* Saves image to disk */
    eResult = stbi_write_bmp_to_func(&orxDisplay_GLFW_WriteResourceCallback,
                                     pstInfo->hResource, pstInfo->u32Width,
                                     pstInfo->u32Height, 4,
                                     pstInfo->pu8ImageData) != 0
                  ? orxSTATUS_SUCCESS
                  : orxSTATUS_FAILURE;
  }
  /* TGA */
    ...
    /* Saves image to disk */
    eResult = stbi_write_tga_to_func(&orxDisplay_GLFW_WriteResourceCallback,
                                     pstInfo->hResource, pstInfo->u32Width,
                                     pstInfo->u32Height, 4,
                                     pstInfo->pu8ImageData) != 0
                  ? orxSTATUS_SUCCESS
...
static orxSTATUS orxFASTCALL
orxDisplay_GLFW_LoadBitmapData(orxBITMAP *_pstBitmap) {
  orxHANDLE hResource;
  orxSTATUS eResult = orxSTATUS_FAILURE;
  ...
  if (hResource != orxHANDLE_UNDEFINED) {
    ...
    /* Allocates buffer */
    pu8Buffer =
        (orxU8 *)orxMemory_Allocate((orxU32)s64Size, orxMEMORY_TYPE_TEMP);
    ...
    if (pu8Buffer != orxNULL) {
      ...
      if (sstDisplay.pstTempBitmap != orxNULL) {
        ...
        uiHeaderSize =
            orxMAX((unsigned int)sizeof(basist::ktx2_header), QOI_HEADER_SIZE);
        ...
        if (((qoi_read_32(pu8Header, &iDummy) == QOI_MAGIC) &&
             (iWidth = qoi_read_32(pu8Header, &iDummy),
              ...
              (iDummy == 3) || (iDummy == 4))) ||
            (orxDisplay_GetBasisUInfo(
                 pu8Header, uiHeaderSize, sstDisplay.eBasisUFormat,
                 (unsigned int *)&iWidth, (unsigned int *)&iHeight,
                 (unsigned int *)&iDummy) != 0) ||
            (stbi_info_from_callbacks(&(sstDisplay.stSTBICallbacks),
                                      (void *)hResource, &iWidth, &iHeight,
                                      &iDummy) != 0)) {
          /* Resets resource cursor */
          orxResource_Seek(hResource, 0, orxSEEK_OFFSET_WHENCE_START);

          /* Updates asynchronous loading flag */
          orxFLAG_SET(_pstBitmap->u32Flags, orxDISPLAY_KU32_BITMAP_FLAG_LOADING,
                      orxDISPLAY_KU32_BITMAP_FLAG_NONE);

          /* Loads data from resource */
          s64Size = orxResource_Read(hResource, s64Size, pu8Buffer,
                                     orxDisplay_GLFW_ReadResourceCallback,
                                     (void *)_pstBitmap);

          /* Successful asynchronous call? */
          if (s64Size < 0) {
            /* Not a cursor nor an icon? */
            if (!orxFLAG_TEST(_pstBitmap->u32Flags,
                              orxDISPLAY_KU32_BITMAP_FLAG_CURSOR |
                                  orxDISPLAY_KU32_BITMAP_FLAG_ICON)) {
              ...
              _pstBitmap->u32RealHeight =
                  sstDisplay.pstTempBitmap->u32RealHeight;
              _pstBitmap->u32Depth = sstDisplay.pstTempBitmap->u32Depth;
              _pstBitmap->fRecRealWidth =
                  sstDisplay.pstTempBitmap->fRecRealWidth;
              _pstBitmap->fRecRealHeight =
                  sstDisplay.pstTempBitmap->fRecRealHeight;
              _pstBitmap->u32DataSize = sstDisplay.pstTempBitmap->u32DataSize;
              orxVector_Copy(&(_pstBitmap->stClip.vTL),
                             &(sstDisplay.pstTempBitmap->stClip.vTL));
              orxVector_Copy(&(_pstBitmap->stClip.vBR),
                             &(sstDisplay.pstTempBitmap->stClip.vBR));
            }
        ...
        /* Loads data from resource */
        s64Size =
            orxResource_Read(hResource, s64Size, pu8Buffer, orxNULL, orxNULL);

        /* Success? */
        if (s64Size != 0) {
          /* Processes data */
          orxDisplay_GLFW_ReadResourceCallback(
              hResource, s64Size, (void *)pu8Buffer, (void *)_pstBitmap);
...
static void orxFASTCALL
orxDisplay_GLFW_DeleteBitmapData(orxBITMAP *_pstBitmap) {
  orxS32 i;
...
static orxSTATUS orxFASTCALL
orxDisplay_GLFW_CompileShader(orxDISPLAY_SHADER *_pstShader) {
  ...
  glShaderSourceARB(hVertexShader, 1, (const GLchar **)&szVertexShaderSource,
                    NULL);
  glASSERT();
  glShaderSourceARB(hFragmentShader, 1, (const GLchar **)&(_pstShader->zCode),
                    NULL);
  glASSERT();
  ...
  if (iSuccess != GL_FALSE) {
    ...
    if (iSuccess != GL_FALSE) {
      ...
      /* Binds attributes */
      glBindAttribLocationARB(hProgram, orxDISPLAY_ATTRIBUTE_LOCATION_VERTEX,
                              "_vPosition_");
      glASSERT();
      glBindAttribLocationARB(hProgram, orxDISPLAY_ATTRIBUTE_LOCATION_TEXCOORD,
                              "_vTexCoord_");
      glASSERT();
      glBindAttribLocationARB(hProgram, orxDISPLAY_ATTRIBUTE_LOCATION_COLOR,
                              "_vColor_");
      glASSERT();
      ...
      _pstShader->iTextureLocation =
          glGetUniformLocationARB(hProgram, "orxTexture");
      glASSERT();

      /* Gets projection matrix location */
      _pstShader->iProjectionMatrixLocation =
          glGetUniformLocationARB(hProgram, "_mProjection_");
      glASSERT();
      ...
      if (iSuccess != GL_FALSE) {
        ...
        /* Gets log */
        glGetProgramInfoLog(hProgram, sizeof(acBuffer) - 1, NULL,
                            (GLchar *)acBuffer);
        glASSERT();
        ...
        /* Outputs log */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                       "Couldn't link shader program:%s%s%s", orxSTRING_EOL,
                       acBuffer, orxSTRING_EOL);
      ...
      /* Gets log */
      glGetShaderInfoLog(hFragmentShader, sizeof(acBuffer) - 1, NULL,
                         (GLchar *)acBuffer);
      glASSERT();
      acBuffer[sizeof(acBuffer) - 1] = orxCHAR_NULL;
      ...
      /* Outputs log */
      orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                     "Couldn't compile fragment shader:%s%s%s", orxSTRING_EOL,
                     acBuffer, orxSTRING_EOL);
    ...
    /* Gets log */
    glGetShaderInfoLog(hVertexShader, sizeof(acBuffer) - 1, NULL,
                       (GLchar *)acBuffer);
    glASSERT();
    acBuffer[sizeof(acBuffer) - 1] = orxCHAR_NULL;
    ...
    /* Outputs log */
    orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                   "Couldn't compile vertex shader:%s%s%s", orxSTRING_EOL,
                   acBuffer, orxSTRING_EOL);
...
static void orxFASTCALL
orxDisplay_GLFW_InitShader(orxDISPLAY_SHADER *_pstShader) {
  /* Uses its program */
  ...
  if (_pstShader->iTextureCount > 0) {
    ...
    for (i = 0; i < _pstShader->iTextureCount; i++) {
      ...
      glUNIFORM(1iARB, _pstShader->astTextureInfoList[i].iLocation,
                sstDisplay.s32ActiveTextureUnit);
    }
  }
...
static void orxFASTCALL orxDisplay_GLFW_DrawArrays() {
  ...
  if (sstDisplay.s32BufferIndex > 0) {
      ...
      if (sstDisplay.eLastBufferMode == orxDISPLAY_BUFFER_MODE_INDIRECT) {
        ...
        glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0,
                           sstDisplay.s32BufferIndex *
                               sizeof(orxDISPLAY_GLFW_VERTEX),
                           sstDisplay.astVertexList);
        glASSERT();
      }
    } else {
    ...
    if (orxLinkList_GetCount(&(sstDisplay.stActiveShaderList)) > 0) {
      ...
      for (pstShader = (orxDISPLAY_SHADER *)orxLinkList_GetFirst(
               &(sstDisplay.stActiveShaderList));
           pstShader != orxNULL; pstShader = pstNextShader) {
        /* Inits shader */
        orxDisplay_GLFW_InitShader(pstShader);

        /* Draws elements */
        glDrawElements(sstDisplay.ePrimitive,
                       (GLsizei)sstDisplay.s32ElementNumber, GL_UNSIGNED_SHORT,
                       pIndexContext);
        glASSERT();

        /* Gets next shader */
        pstNextShader =
            (orxDISPLAY_SHADER *)orxLinkList_GetNext(&(pstShader->stNode));
    ...
    sstDisplay.s32BufferIndex = sstDisplay.s32ElementNumber = 0;
...
static void orxFASTCALL
orxDisplay_GLFW_SetBufferMode(orxDISPLAY_BUFFER_MODE _eBufferMode) {
  /* New buffer mode? */
    ...
    if (_eBufferMode == orxDISPLAY_BUFFER_MODE_INDIRECT) {
      ...
      if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO)) {
        ...
        glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                        orxDISPLAY_KU32_VERTEX_BUFFER_SIZE *
                            sizeof(orxDISPLAY_GLFW_VERTEX),
        ...
        if (orxFLAG_TEST(sstDisplay.u32Flags,
                         orxDISPLAY_KU32_STATIC_FLAG_CUSTOM_IBO)) {
          /* Fills IBO */
          glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
                          orxDISPLAY_KU32_INDEX_BUFFER_SIZE * sizeof(GLushort),
                          sstDisplay.au16IndexList, GL_STATIC_DRAW_ARB);
          glASSERT();
...
static void orxFASTCALL orxDisplay_GLFW_PrepareBitmap(
    const orxBITMAP *_pstBitmap, orxDISPLAY_SMOOTHING _eSmoothing,
    orxDISPLAY_BLEND_MODE _eBlendMode, orxDISPLAY_BUFFER_MODE _eBufferMode) {
  orxBOOL bSmoothing;
  ...
  if (_pstBitmap !=
    ...
    if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_SHADER)) {
      ...
      if (orxLinkList_GetCount(&(sstDisplay.stActiveShaderList)) == 0) {
        /* Updates shader uniform */
        glUNIFORM(1iARB, sstDisplay.pstDefaultShader->iTextureLocation,
                  sstDisplay.s32ActiveTextureUnit);
      }
    }
  }
  ...
  if (bSmoothing ^
      (orxFLAG_TEST(_pstBitmap->u32Flags, orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING)
           ? orxTRUE
           : orxFALSE)) {
    /* Draws remaining items */
    orxDisplay_GLFW_DrawArrays();
    ...
    if (bSmoothing != orxFALSE) {
      ...
      /* Updates mode */
      orxFLAG_SET(((orxBITMAP *)_pstBitmap)->u32Flags,
                  orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING,
                  orxDISPLAY_KU32_BITMAP_FLAG_NONE);
    } else {
      /* Updates texture */
...
static orxINLINE void
orxDisplay_GLFW_DrawBitmap(const orxBITMAP *_pstBitmap,
                           const orxDISPLAY_MATRIX *_pmTransform,
  ...
  orxDisplay_GLFW_PrepareBitmap(_pstBitmap, _eSmoothing, _eBlendMode,
                                orxDISPLAY_BUFFER_MODE_INDIRECT);
  ...
  sstDisplay.astVertexList[sstDisplay.s32BufferIndex].stRGBA =
      sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].stRGBA =
          sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].stRGBA =
              sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3].stRGBA =
                  _stColor;
...
static void orxFASTCALL orxDisplay_GLFW_DrawPrimitive(
  ...
  if (_eBlendMode != orxDISPLAY_BLEND_MODE_ALPHA ||
    ...
    orxDisplay_GLFW_SetBlendMode(orxDISPLAY_BLEND_MODE_NONE);
  }
  ...
  if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO)) {
    /* Copies vertex buffer */
    glBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0,
                       _u32VertexNumber * sizeof(orxDISPLAY_GLFW_VERTEX),
                       sstDisplay.astVertexList);
    glASSERT();
  }
...
static orxINLINE GLenum
orxDisplay_GLFW_GetOpenGLPrimitive(orxDISPLAY_PRIMITIVE _ePrimitive) {
  GLenum eResult;

#define orxDISPLAY_PRIMITIVE_CASE(TYPE)                                        \
  case orxDISPLAY_PRIMITIVE_##TYPE:                                            \
    eResult = GL_##TYPE;                                                       \
    break
  ...
  default:
    eResult = orxDISPLAY_KE_DEFAULT_PRIMITIVE;
    break;
  }
...
static orxINLINE void orxDisplay_GLFW_UpdateCursor() {
  ...
  if (*zCursor != orxCHAR_NULL) {
    ...
    for (i = 0, bFound = orxFALSE;
         i < orxARRAY_GET_ITEM_COUNT(sastStandardCursorList); i++) {
      /* Matches? */
      if (orxString_ICompare(zCursor, sastStandardCursorList[i].zName) == 0) {
        ...
        if (sastStandardCursorList[i].iShape != 0) {
          /* Creates cursor */
          sstDisplay.pstCursor =
              glfwCreateStandardCursor(sastStandardCursorList[i].iShape);
    ...
    if (bFound == orxFALSE) {
      ...
      /* Locates resource */
      zResourceLocation =
          orxResource_Locate(orxTEXTURE_KZ_RESOURCE_GROUP, zCursor);
      ...
      if (zResourceLocation != orxNULL) {
        ...
        if (pstBitmap != orxNULL) {
          ...
          if (orxConfig_GetListCount(orxDISPLAY_KZ_CONFIG_CURSOR) > 1) {
            ...
            /* Gets it */
            if (orxConfig_GetListVector(orxDISPLAY_KZ_CONFIG_CURSOR, 1,
                                        &vPivot) != orxNULL) {
              /* Stores it */
    ...
    if (s32Count > 0) {
      ...
      if (s32Count > orxDISPLAY_KU32_MAX_ICON_NUMBER) {
        ...
        orxDEBUG_PRINT(
            ...
            "Can't set <%d> icons: only the first <%u> will be used.", s32Count,
            orxDISPLAY_KU32_MAX_ICON_NUMBER);
      ...
      for (i = 0, sstDisplay.s32IconNumber = 0; i < s32Count; i++) {
        ...
        zResourceLocation =
            orxResource_Locate(orxTEXTURE_KZ_RESOURCE_GROUP, zIcon);
        ...
        if (zResourceLocation != orxNULL) {
          ...
          if (pstBitmap != orxNULL) {
            ...
            /* Loads bitmap's data */
            if (orxDisplay_GLFW_LoadBitmapData(pstBitmap) ==
                orxSTATUS_FAILURE) {
              /* Updates pending icon count */
              sstDisplay.s32PendingIconCount--;
 ...
 */
static orxSTATUS orxFASTCALL
orxDisplay_GLFW_EventHandler(const orxEVENT *_pstEvent) {
  orxSTATUS eResult = orxSTATUS_SUCCESS;
  ...
  case orxEVENT_TYPE_SYSTEM: {
    ...
    if (pstPayload != orxNULL) {
      ...
      if (pstPayload->stClipboard.zValue == orxNULL) {
        /* Updates payload */
        pstPayload->stClipboard.zValue =
            glfwGetClipboardString(sstDisplay.pstWindow);
      }
      /* Set */
      else {
        /* Updates clipboard */
        glfwSetClipboardString(sstDisplay.pstWindow,
                               pstPayload->stClipboard.zValue);
      }

      /* Updates result */
      eResult = (glfwGetError(NULL) == GLFW_NO_ERROR) ? orxSTATUS_SUCCESS
                                                      : orxSTATUS_FAILURE;
    } else {
      /* Updates result */
      eResult = orxSTATUS_FAILURE;
 ...
 */
static void orxFASTCALL orxDisplay_GLFW_VSyncFix(const orxCLOCK_INFO *_pstInfo,
                                                 void *_pContext) {
  /* Updates status */
  orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VSYNC_FIX,
              orxDISPLAY_KU32_STATIC_FLAG_NONE);
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_TransformText(
  ...
  /* For all characters */
  for (u32CharacterCodePoint =
           orxString_GetFirstCharacterCodePoint(_zString, &pc),
      fX = 0.0f, fY = 0.0f;
       (u32CharacterCodePoint != orxCHAR_NULL) &&
       (u32CharacterCodePoint != orxU32_UNDEFINED);
    ...
    default: {
      ...
      if (pstGlyph != orxNULL) {
        ...
        if (sstDisplay.s32BufferIndex >
            orxDISPLAY_KU32_VERTEX_BUFFER_SIZE - 5) {
          /* Draws arrays */
          orxDisplay_GLFW_DrawArrays();
        }
        ...
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex].fX =
            (mTransform.vX.fX * fX) + (mTransform.vX.fY * (fY + fHeight)) +
            mTransform.vX.fZ;
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex].fY =
            (mTransform.vY.fX * fX) + (mTransform.vY.fY * (fY + fHeight)) +
            mTransform.vY.fZ;
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].fX =
        ...
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].fX =
            ...
            (mTransform.vX.fY * (fY + fHeight)) + mTransform.vX.fZ;
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].fY =
            (mTransform.vY.fX * (fX + fWidth)) +
            (mTransform.vY.fY * (fY + fHeight)) + mTransform.vY.fZ;
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3].fX =
            (mTransform.vX.fX * (fX + fWidth)) + (mTransform.vX.fY * fY) +
            mTransform.vX.fZ;
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3].fY =
            (mTransform.vY.fX * (fX + fWidth)) + (mTransform.vY.fY * fY) +
            mTransform.vY.fZ;

        sstDisplay.astVertexList[sstDisplay.s32BufferIndex].fU =
            sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].fU =
                (GLfloat)(_pstFont->fRecRealWidth *
                          (pstGlyph->fX + _pstFont->fBorderFix));
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].fU =
            sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3].fU =
                (GLfloat)(_pstFont->fRecRealWidth *
                          (pstGlyph->fX + fWidth - _pstFont->fBorderFix));
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].fV =
            sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3].fV =
                (GLfloat)(_pstFont->fRecRealHeight *
                          (pstGlyph->fY + _pstFont->fBorderFix));
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex].fV =
            sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].fV =
                (GLfloat)(_pstFont->fRecRealHeight *
                          (pstGlyph->fY + fHeight - _pstFont->fBorderFix));

        /* Fills the color list */
        sstDisplay.astVertexList[sstDisplay.s32BufferIndex].stRGBA =
            sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].stRGBA =
                sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2].stRGBA =
                    sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3]
                        .stRGBA = _stColor;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawLine(const orxVECTOR *_pvStart,
  ...
  /* Copies color */
  sstDisplay.astVertexList[0].stRGBA = sstDisplay.astVertexList[1].stRGBA =
      _stColor;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawPolyline(
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawPolygon(
    const orxVECTOR *_avVertexList, orxU32 _u32VertexNumber, orxRGBA _stColor,
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawCircle(const orxVECTOR *_pvCenter,
                                                 orxFLOAT _fRadius,
                                                 orxRGBA _stColor,
                                                 orxBOOL _bFill) {
  orxU32 i;
  ...
  /* For all vertices */
  for (i = 0, fAngle = orxFLOAT_0; i < orxDISPLAY_KU32_CIRCLE_LINE_NUMBER;
       i++, fAngle += orxMATH_KF_2_PI / orxDISPLAY_KU32_CIRCLE_LINE_NUMBER) {
    /* Copies its coords */
    sstDisplay.astVertexList[i].fX =
        (GLfloat)(_fRadius * orxMath_Cos(fAngle) + _pvCenter->fX);
    sstDisplay.astVertexList[i].fY =
        (GLfloat)(_fRadius * orxMath_Sin(fAngle) + _pvCenter->fY);
  ...
  /* Draws it */
  orxDisplay_GLFW_DrawPrimitive(orxDISPLAY_KU32_CIRCLE_LINE_NUMBER, _stColor,
                                _bFill, orxFALSE, orxDISPLAY_BLEND_MODE_ALPHA);

  /* Done! */
  return eResult;
}

orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawOBox(const orxOBOX *_pstBox,
                                               orxRGBA _stColor,
                                               orxBOOL _bFill) {
  orxVECTOR vOrigin;
  orxSTATUS eResult = orxSTATUS_SUCCESS;
  ...
  sstDisplay.astVertexList[2].fX =
      (GLfloat)(vOrigin.fX + _pstBox->vX.fX + _pstBox->vY.fX);
  sstDisplay.astVertexList[2].fY =
      (GLfloat)(vOrigin.fY + _pstBox->vX.fY + _pstBox->vY.fY);
  ...
  /* Copies color */
  sstDisplay.astVertexList[0].stRGBA = sstDisplay.astVertexList[1].stRGBA =
      sstDisplay.astVertexList[2].stRGBA = sstDisplay.astVertexList[3].stRGBA =
          _stColor;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_DrawMesh(
  ...
  orxASSERT((_pstMesh->au16IndexList == orxNULL) ||
            (_pstMesh->u32IndexNumber > 1));
  orxASSERT((_pstMesh->ePrimitive < orxDISPLAY_PRIMITIVE_NUMBER) ||
            ((_pstMesh->ePrimitive == orxDISPLAY_PRIMITIVE_NONE) &&
             (_pstMesh->au16IndexList == orxNULL)));
  ...
  /* Stores primitive */
  sstDisplay.ePrimitive =
      orxDisplay_GLFW_GetOpenGLPrimitive(_pstMesh->ePrimitive);

  /* Gets element number */
  u32ElementNumber =
      ((_pstMesh->u32IndexNumber != 0) && (_pstMesh->au16IndexList != orxNULL))
          ? _pstMesh->u32IndexNumber
          : _pstMesh->u32VertexNumber + (_pstMesh->u32VertexNumber >> 1);

  /* Has VBO support? */
  if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO)) {
    /* Fills VBO */
    glBufferDataARB(GL_ARRAY_BUFFER_ARB,
                    _pstMesh->u32VertexNumber * sizeof(orxDISPLAY_GLFW_VERTEX),
                    _pstMesh->astVertexList, GL_STREAM_DRAW_ARB);
    glASSERT();
    ...
    if ((_pstMesh->au16IndexList != orxNULL) &&
      ...
      /* Fills IBO */
      glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
                      _pstMesh->u32IndexNumber * sizeof(GLushort),
                      _pstMesh->au16IndexList, GL_STREAM_DRAW_ARB);
      glASSERT();

      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_CUSTOM_IBO,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    }
    ...
    glVertexPointer(2, GL_FLOAT, sizeof(orxDISPLAY_VERTEX),
                    &(_pstMesh->astVertexList[0].fX));
    glASSERT();
    glTexCoordPointer(2, GL_FLOAT, sizeof(orxDISPLAY_VERTEX),
                      &(_pstMesh->astVertexList[0].fU));
    glASSERT();
    glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(orxDISPLAY_VERTEX),
                   &(_pstMesh->astVertexList[0].stRGBA));
    glASSERT();

    /* Draws mesh */
    glDrawElements(sstDisplay.ePrimitive, (GLsizei)u32ElementNumber,
                   GL_UNSIGNED_SHORT,
                   (GLvoid *)((_pstMesh->au16IndexList != orxNULL)
                                  ? _pstMesh->au16IndexList
...
void orxFASTCALL orxDisplay_GLFW_DeleteBitmap(orxBITMAP *_pstBitmap) {
  ...
  if (_pstBitmap != sstDisplay.pstScreen) {
    ...
    if (orxFLAG_TEST(_pstBitmap->u32Flags,
                     orxDISPLAY_KU32_BITMAP_FLAG_LOADING)) {
      /* Asks for deletion */
      orxFLAG_SET(_pstBitmap->u32Flags, orxDISPLAY_KU32_BITMAP_FLAG_DELETE,
                  orxDISPLAY_KU32_BITMAP_FLAG_NONE);
    } else {
      /* Deletes its data */
...
orxBITMAP *orxFASTCALL orxDisplay_GLFW_CreateBitmap(orxU32 _u32Width,
                                                    orxU32 _u32Height) {
  orxBITMAP *pstBitmap;
  ...
  if (pstBitmap != orxNULL) {
    ...
    pstBitmap->fBorderFix = ((_u32Width > 2) && (_u32Height > 2))
                                ? orxDISPLAY_KF_BORDER_FIX
                                : orxFLOAT_0;
    ...
    pstBitmap->u32DataSize =
        pstBitmap->u32RealWidth * pstBitmap->u32RealHeight * 4 * sizeof(orxU8);
    pstBitmap->zLocation = orxSTRING_EMPTY;
    pstBitmap->stFilenameID = orxSTRINGID_UNDEFINED;
    pstBitmap->u32Flags = (sstDisplay.bDefaultSmoothing != orxFALSE)
                              ? orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING
                              : orxDISPLAY_KU32_BITMAP_FLAG_NONE;
    ...
    glASSERT();
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)pstBitmap->u32RealWidth,
                 (GLsizei)pstBitmap->u32RealHeight, 0, GL_RGBA,
    ...
    glASSERT();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                    (sstDisplay.bDefaultSmoothing != orxFALSE) ? GL_LINEAR
                                                               : GL_NEAREST);
    glASSERT();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                    (sstDisplay.bDefaultSmoothing != orxFALSE) ? GL_LINEAR
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_ClearBitmap(orxBITMAP *_pstBitmap,
                                                  orxRGBA _stColor) {
  orxSTATUS eResult = orxSTATUS_SUCCESS;
  ...
  if (_pstBitmap == orxNULL) {
    ...
    if (_stColor.u32RGBA != sstDisplay.stLastColor.u32RGBA) {
      /* Updates it */
      glClearColor(orxCOLOR_NORMALIZER * orxU2F(orxRGBA_R(_stColor)),
                   orxCOLOR_NORMALIZER * orxU2F(orxRGBA_G(_stColor)),
                   orxCOLOR_NORMALIZER * orxU2F(orxRGBA_B(_stColor)),
                   orxCOLOR_NORMALIZER * orxU2F(orxRGBA_A(_stColor)));
      glASSERT();
    ...
    orxMemory_Copy(apstBackupBitmap, sstDisplay.apstDestinationBitmapList,
                   sstDisplay.u32DestinationBitmapCount * sizeof(orxBITMAP *));
    u32BackupBitmapCount = sstDisplay.u32DestinationBitmapCount;

    /* Sets new destination bitmap */
    if (orxDisplay_GLFW_SetDestinationBitmaps(&_pstBitmap, 1) !=
        orxSTATUS_FAILURE) {
      /* Different clear color? */
      if (_stColor.u32RGBA != sstDisplay.stLastColor.u32RGBA) {
        /* Updates it */
      ...
      orxDisplay_GLFW_SetDestinationBitmaps(apstBackupBitmap,
                                            u32BackupBitmapCount);
    }
    /* Not screen? */
      ...
      /* For all pixels */
      for (pstPixel = astBuffer;
           pstPixel <
           astBuffer + (_pstBitmap->u32RealWidth * _pstBitmap->u32RealHeight);
           pstPixel++) {
        /* Sets its value */
        *pstPixel = _stColor;
      }
      ...
      /* Updates texture */
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, (GLsizei)_pstBitmap->u32RealWidth,
                      (GLsizei)_pstBitmap->u32RealHeight, GL_RGBA,
                      GL_UNSIGNED_BYTE, astBuffer);
      glASSERT();
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetBitmapData(orxBITMAP *_pstBitmap,
  ...
  if (!orxFLAG_TEST(_pstBitmap->u32Flags,
    ...
    /* Valid? */
    if ((_pstBitmap != sstDisplay.pstScreen) &&
        (_u32ByteNumber == u32Width * u32Height * 4 * sizeof(orxU8))) {
      /* Binds texture */
      glBindTexture(GL_TEXTURE_2D, _pstBitmap->uiTexture);
      glASSERT();
      ...
      if (_pstBitmap == sstDisplay.pstScreen) {
        ...
        orxDEBUG_PRINT(
            orxDEBUG_LEVEL_DISPLAY,
            "Can't set bitmap data for [%s]: format needs to be RGBA.",
            _pstBitmap->zLocation);
      }
    ...
    orxDEBUG_PRINT(
        orxDEBUG_LEVEL_DISPLAY,
        "Can't set bitmap data for [%s]: bitmap is not done loading.",
        _pstBitmap->zLocation);
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_GetBitmapData(const orxBITMAP *_pstBitmap,
                                                    orxU8 *_au8Data,
                                                    orxU32 _u32ByteNumber) {
  orxSTATUS eResult;
  ...
  if (!orxFLAG_TEST(_pstBitmap->u32Flags,
    ...
    /* Gets buffer size */
    u32BufferSize =
        orxF2U(_pstBitmap->fWidth * _pstBitmap->fHeight) * 4 * sizeof(orxU8);
    ...
    if (_u32ByteNumber == u32BufferSize) {
...

      {
        ...
        /* Backups current destinations */
        orxMemory_Copy(apstBackupBitmap, sstDisplay.apstDestinationBitmapList,
                       sstDisplay.u32DestinationBitmapCount *
                           sizeof(orxBITMAP *));
        u32BackupBitmapCount = sstDisplay.u32DestinationBitmapCount;

        /* Sets new destination bitmap */
        eResult =
            orxDisplay_GLFW_SetDestinationBitmaps((orxBITMAP **)&_pstBitmap, 1);
        ...
        /* Allocates buffer */
        pu8ImageBuffer =
            (_pstBitmap != sstDisplay.pstScreen)
                ? _au8Data
                : (orxU8 *)orxMemory_Allocate(_pstBitmap->u32RealWidth *
                                                  _pstBitmap->u32RealHeight *
                                                  4 * sizeof(orxU8),
                                              orxMEMORY_TYPE_TEMP);
        ...
        /* Reads OpenGL data */
        glReadPixels(0, 0, _pstBitmap->u32RealWidth, _pstBitmap->u32RealHeight,
      ...
      if (_pstBitmap == sstDisplay.pstScreen) {
        ...
        glASSERT();
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
                     (GLsizei)sstDisplay.pstScreen->u32RealWidth,
                     (GLsizei)sstDisplay.pstScreen->u32RealHeight, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE, NULL);
        glASSERT();
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glASSERT();
        ...
        glASSERT();
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                        orxFLAG_TEST(sstDisplay.pstScreen->u32Flags,
                                     orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING)
                            ? GL_LINEAR
                            : GL_NEAREST);
        glASSERT();
        ...
        glCopyTexSubImage2D(
            ...
            (GLint)(orxF2U(_pstBitmap->fHeight) - _pstBitmap->u32RealHeight),
            (GLsizei)orxF2U(_pstBitmap->fWidth),
            (GLsizei)_pstBitmap->u32RealHeight);
        glASSERT();
      ...
      if (_pstBitmap == sstDisplay.pstScreen) {
        ...
        /* For all lines */
        for (i = 0,
            u32SrcOffset = u32RealLineSize * (_pstBitmap->u32RealHeight -
                                              orxF2U(_pstBitmap->fHeight)),
            u32DstOffset = u32LineSize * (orxF2U(_pstBitmap->fHeight) - 1);
             i < orxF2U(_pstBitmap->fHeight); i++,
            u32SrcOffset += u32RealLineSize, u32DstOffset -= u32LineSize) {
          orxU32 j;
          ...
          for (j = 0; j < orxF2U(_pstBitmap->fWidth); j++) {
            ...
            /* Gets opaque pixel */
            stPixel.u32RGBA =
                ((orxRGBA *)(pu8ImageBuffer + u32SrcOffset))[j].u32RGBA |
                stOpaque.u32RGBA;
      ...
      /* Logs message */
      orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                     "Can't get bitmap data for [%s] as the buffer size is "
                     "<%u> when it should be <%u>.",
                     _pstBitmap->zLocation, _u32ByteNumber, u32BufferSize);
    ...
    /* Logs message */
    orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                   "Can't get bitmap data for [%s] as it's not done loading.",
                   _pstBitmap->zLocation);
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetPartialBitmapData(
  ...
  if (!orxFLAG_TEST(_pstBitmap->u32Flags,
    ...
    /* Valid? */
    if ((_pstBitmap != sstDisplay.pstScreen) &&
        (_u32X + _u32Width <= u32BitmapWidth) &&
        (_u32Y + _u32Height <= u32BitmapHeight)) {
      orxU8 *pu8ImageBuffer;
      ...
      /* Updates its content */
      glTexSubImage2D(GL_TEXTURE_2D, 0, _u32X, _u32Y, (GLsizei)_u32Width,
                      (GLsizei)_u32Height, GL_RGBA, GL_UNSIGNED_BYTE,
                      pu8ImageBuffer);
      glASSERT();
      ...
      if (_pstBitmap == sstDisplay.pstScreen) {
        ...
        /* Logs message */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                       "Can't set bitmap data for [%s]: rectangle coordinates "
                       "(%u, %u) - (%u, %u) are out of bound [%ux%u].",
                       _pstBitmap->zLocation, _u32X, _u32Y, _u32X + _u32Width,
                       _u32Y + _u32Height, u32BitmapWidth, u32BitmapHeight);
      }
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetDestinationBitmaps(
  ...
  if (orxFLAG_TEST(sstDisplay.u32Flags,
    ...
    if (_u32Number > (orxU32)sstDisplay.iDrawBufferNumber) {
      /* Outputs logs */
      orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                     "Can only attach the first <%d> bitmaps as destinations, "
                     "out of the <%u> requested.",
                     sstDisplay.iDrawBufferNumber, _u32Number);
    ...
    if (u32Number != 0) {
      ...
      /* Updates draw status */
      bDraw =
          ((_apstBitmapList[0] != sstDisplay.apstDestinationBitmapList[0]) ||
           (u32Number != sstDisplay.u32DestinationBitmapCount))
              ? orxTRUE
              : orxFALSE;

      /* Not screen? */
      if ((_apstBitmapList[0] != orxNULL) &&
          (_apstBitmapList[0] != sstDisplay.pstScreen)) {
        orxFLOAT fWidth, fHeight;
        ...
        for (i = 1; (i < u32Number) && (eResult != orxSTATUS_FAILURE); i++) {
          ...
          orxASSERT((pstBitmap != sstDisplay.pstScreen) &&
                    "Can only use screen as bitmap destination by itself.");

          /* Valid? */
          if (pstBitmap != orxNULL) {
            /* Same size? */
            if ((pstBitmap->fWidth == fWidth) &&
                (pstBitmap->fHeight == fHeight)) {
              ...
              /* Logs message */
              orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                             "Can't set bitmap destinations as they have "
                             "different dimensions: (%f, %f) != (%f, %f).",
                             pstBitmap->fWidth, pstBitmap->fHeight, fWidth,
                             fHeight);
      ...
      if (eResult != orxSTATUS_FAILURE) {
        ...
        for (i = 0; i < u32Number; i++) {
          ...
          /* Valid texture? */
            ...
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
                                      ...
                                      GL_TEXTURE_2D, pstBitmap->uiTexture, 0);
            glASSERT();
          } else {
            /* Updates result */
            eResult = orxSTATUS_FAILURE;
        ...
        /* Updates result */
        eResult = (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT) ==
    ...
    if ((_u32Number == 1) && (_apstBitmapList[0] == sstDisplay.pstScreen)) {
      ...
      if (_u32Number != 0) {
        /* Outputs logs */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                       "Can only attach screen as destination as there's no "
                       "framebuffer support on this machine.");
  ...
  if (eResult != orxSTATUS_FAILURE) {
    ...
    if (sstDisplay.apstDestinationBitmapList[0] == sstDisplay.pstScreen) {
      ...
      glfwGetFramebufferSize(sstDisplay.pstWindow, (int *)&iWidth,
                             (int *)&iHeight);
      ...
      iHeight =
          (GLsizei)orxF2S(sstDisplay.apstDestinationBitmapList[0]->fHeight);
    ...
    if ((fOrthoRight != sstDisplay.fLastOrthoRight) ||
      ...
      (sstDisplay.fLastOrthoBottom >= 0.0)
          ? orxDisplay_GLFW_OrthoProjMatrix(
                &(sstDisplay.mProjectionMatrix), orxFLOAT_0, fOrthoRight,
                fOrthoBottom, orxFLOAT_0, -orxFLOAT_1, orxFLOAT_1)
          : orxDisplay_GLFW_OrthoProjMatrix(
                &(sstDisplay.mProjectionMatrix), orxFLOAT_0, fOrthoRight,
                orxFLOAT_0, -fOrthoBottom, -orxFLOAT_1, orxFLOAT_1);
      ...
      if (orxFLAG_TEST(sstDisplay.u32Flags,
        ...
        glUNIFORM(
            Matrix4fvARB,
            sstDisplay.pstDefaultShader->iProjectionMatrixLocation, 1, GL_FALSE,
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_TransformBitmap(
  ...
  if (_pstSrc == orxNULL) {
    ...
    sstDisplay.astVertexList[0].fX = sstDisplay.astVertexList[1].fX =
        sstDisplay.apstDestinationBitmapList[0]->stClip.vTL.fX;
    sstDisplay.astVertexList[2].fX = sstDisplay.astVertexList[3].fX =
        sstDisplay.apstDestinationBitmapList[0]->stClip.vBR.fX;
    sstDisplay.astVertexList[1].fY = sstDisplay.astVertexList[3].fY =
        sstDisplay.apstDestinationBitmapList[0]->stClip.vTL.fY;
    sstDisplay.astVertexList[0].fY = sstDisplay.astVertexList[2].fY =
        sstDisplay.apstDestinationBitmapList[0]->stClip.vBR.fY;

    /* Defines the texture coord list */
    sstDisplay.astVertexList[0].fU = sstDisplay.astVertexList[1].fU = 0.0f;
    sstDisplay.astVertexList[2].fU = sstDisplay.astVertexList[3].fU = 1.0f;
    sstDisplay.astVertexList[1].fV = sstDisplay.astVertexList[3].fV = 0.0f;
    sstDisplay.astVertexList[0].fV = sstDisplay.astVertexList[2].fV = 1.0f;
    ...
    /* No repeat? */
    if ((_pstTransform->fRepeatX == orxFLOAT_1) &&
        (_pstTransform->fRepeatY == orxFLOAT_1)) {
      /* Draws it */
      orxDisplay_GLFW_DrawBitmap(_pstSrc, &mTransform, _stColor, _eSmoothing,
                                 _eBlendMode);
    } else {
      ...
      fLeft = _pstSrc->fRecRealWidth *
              (_pstSrc->stClip.vTL.fX + _pstSrc->fBorderFix);
      fTop = _pstSrc->fRecRealHeight *
             (_pstSrc->stClip.vTL.fY + _pstSrc->fBorderFix);
      ...
      /* For all lines */
      for (fY = 0.0f, i = fRepeatY, fRecRepeatX = orxFLOAT_1 / fRepeatX;
           i > orxFLOAT_0; i -= orxFLOAT_1, fY += fHeight) {
        /* Partial line? */
        if (i < orxFLOAT_1) {
          /* Updates height */
          fHeight *= (GLfloat)i;
          ...
          fRight = (GLfloat)(_pstSrc->fRecRealWidth *
                             (_pstSrc->stClip.vBR.fX - _pstSrc->fBorderFix));
          fBottom = (GLfloat)(_pstSrc->fRecRealHeight *
                              (_pstSrc->stClip.vTL.fY +
                               (i * (_pstSrc->stClip.vBR.fY -
                                     _pstSrc->stClip.vTL.fY)) -
                               _pstSrc->fBorderFix));
        } else {
          /* Resets texture coords */
          fRight = (GLfloat)(_pstSrc->fRecRealWidth *
                             (_pstSrc->stClip.vBR.fX - _pstSrc->fBorderFix));
          fBottom = (GLfloat)(_pstSrc->fRecRealHeight *
                              (_pstSrc->stClip.vBR.fY - _pstSrc->fBorderFix));
        }

        /* Resets bitmap width */
        fWidth = (GLfloat)((_pstSrc->stClip.vBR.fX - _pstSrc->stClip.vTL.fX) *
                           fRecRepeatX);

        /* For all columns */
        for (fX = 0.0f, j = fRepeatX; j > orxFLOAT_0;
             j -= orxFLOAT_1, fX += fWidth) {
          /* Partial column? */
          if (j < orxFLOAT_1) {
            /* Updates width */
            fWidth *= (GLfloat)j;
          ...
          sstDisplay.astVertexList[sstDisplay.s32BufferIndex].stRGBA =
              sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 1].stRGBA =
                  sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 2]
                      .stRGBA =
                      sstDisplay.astVertexList[sstDisplay.s32BufferIndex + 3]
                          .stRGBA = _stColor;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SaveBitmap(const orxBITMAP *_pstBitmap,
  ...
  /* Allocates buffer */
  pu8ImageData =
      (orxU8 *)orxMemory_Allocate(u32BufferSize, orxMEMORY_TYPE_TEMP);
  ...
  if (pu8ImageData != orxNULL) {
    ...
    /* Gets bitmap data */
    if (orxDisplay_GLFW_GetBitmapData(_pstBitmap, pu8ImageData,
                                      u32BufferSize) != orxSTATUS_FAILURE) {
      const orxSTRING zResourceLocation;
      orxHANDLE hResource;

      /* Valid file to open? */
      if (((zResourceLocation = orxResource_LocateInStorage(
                orxTEXTURE_KZ_RESOURCE_GROUP, orxRESOURCE_KZ_DEFAULT_STORAGE,
                _zFileName)) != orxNULL) &&
          ((hResource = orxResource_Open(zResourceLocation, orxTRUE)) !=
           orxHANDLE_UNDEFINED)) {
        /* Allocates save info */
        pstInfo = (orxDISPLAY_SAVE_INFO *)orxMemory_Allocate(
            sizeof(orxDISPLAY_SAVE_INFO), orxMEMORY_TYPE_TEMP);
        ...
        if (pstInfo != orxNULL) {
          ...
          /* Runs asynchronous task */
          eResult = orxThread_RunTask(&orxDisplay_GLFW_SaveBitmapData, orxNULL,
                                      orxNULL, (void *)pstInfo);
        } else {
          /* Closes resource */
          orxResource_Close(hResource);
        }
...
orxSTATUS orxFASTCALL
orxDisplay_GLFW_SetTempBitmap(const orxBITMAP *_pstBitmap) {
  orxSTATUS eResult = orxSTATUS_SUCCESS;
...
orxBITMAP *orxFASTCALL orxDisplay_GLFW_LoadBitmap(const orxSTRING _zFileName) {
  ...
  /* Locates resource */
  zResourceLocation =
      orxResource_Locate(orxTEXTURE_KZ_RESOURCE_GROUP, _zFileName);
  ...
  if (zResourceLocation != orxNULL) {
    ...
    if (pstResult != orxNULL) {
      ...
      pstResult->u32Flags = (sstDisplay.bDefaultSmoothing != orxFALSE)
                                ? orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING
                                : orxDISPLAY_KU32_BITMAP_FLAG_NONE;
...
orxBITMAP *orxFASTCALL orxDisplay_GLFW_LoadFont(
      ...
      if (hResource != orxHANDLE_UNDEFINED) {
        ...
        if (pstResult != orxNULL) {
          ...
          if (pu8Buffer != orxNULL) {
            ...
            if (orxResource_Read(hResource, s64Size, pu8Buffer, orxNULL,
                                 orxNULL) == s64Size) {
              orxDISPLAY_FONT_LOAD_INFO *pstLoadInfo;

              /* Allocates font load info */
              pstLoadInfo = (orxDISPLAY_FONT_LOAD_INFO *)orxMemory_Allocate(
                  sizeof(orxDISPLAY_FONT_LOAD_INFO), orxMEMORY_TYPE_TEMP);
              ...
              if (pstLoadInfo != orxNULL) {
                ...
                /* Initializes font */
                if (stbtt_InitFont(&(pstLoadInfo->stFontInfo), pu8Buffer, 0) !=
                    0) {
                  /* Gets glyph count */
                  pstLoadInfo->u32GlyphCount =
                      orxString_GetCharacterCount(_zCharacterList);

                  /* Valid? */
                  if (pstLoadInfo->u32GlyphCount > 0) {
                    /* Allocates glyph list */
                    pstLoadInfo->astGlyphList =
                        (orxDISPLAY_FONT_GLYPH *)orxMemory_Allocate(
                            pstLoadInfo->u32GlyphCount *
                                sizeof(orxDISPLAY_FONT_GLYPH),
                            orxMEMORY_TYPE_TEMP);
                    ...
                    if (pstLoadInfo->astGlyphList != orxNULL) {
                      ...
                      orxFLOAT fCurrentWidth, fWidth, fHeight, fBaseLine,
                          fXPadding, fYPadding, fMaxTextureWidth;
                      ...
                      fMaxTextureWidth =
                          (orxConfig_HasValue(
                           ...
                           orxFALSE)
                              ? orxConfig_GetFloat(
                                    orxDISPLAY_KZ_CONFIG_MAX_TEXTURE_SIZE)
                              : orxS2F(sstDisplay.iMaxTextureSize);
                      orxConfig_PopSection();
                      ...
                      for (u32CharacterCodePoint =
                               orxString_GetFirstCharacterCodePoint(
                                   _zCharacterList, &zCharacterList),
                          i = iX0 = iX1 = iY0 = iY1 = 0;
                           (u32CharacterCodePoint != orxCHAR_NULL) &&
                           (u32CharacterCodePoint != orxU32_UNDEFINED);
                           u32CharacterCodePoint =
                               orxString_GetFirstCharacterCodePoint(
                                   zCharacterList, &zCharacterList),
                          i++) {
                        int iGlyphX0, iGlyphX1, iGlyphY0, iGlyphY1;

                        /* Stores its glyph index */
                        pstLoadInfo->astGlyphList[i].s32Index =
                            stbtt_FindGlyphIndex(&(pstLoadInfo->stFontInfo),
                                                 u32CharacterCodePoint);

                        /* Gets glyph bitmap box */
                        stbtt_GetGlyphBitmapBox(
                            &(pstLoadInfo->stFontInfo),
                            pstLoadInfo->astGlyphList[i].s32Index, 1.0f, 1.0f,
                            (int *)&iGlyphX0, (int *)&iGlyphY0,
                            (int *)&iGlyphX1, (int *)&iGlyphY1);
                      ...
                      /* Gets font scale */
                      pstLoadInfo->vFontScale.fY = orxMAX(
                          orxFLOAT_0, (_pvCharacterSize->fY -
                                       orx2F(2.0f) * fYPadding - orxFLOAT_1) /
                                          (iY1 - iY0));
                      pstLoadInfo->vFontScale.fX =
                          (_pvCharacterSize->fX > orxFLOAT_0)
                              ? orxMAX(orxFLOAT_0,
                                       ((_pvCharacterSize->fX -
                                         orx2F(2.0f) * fXPadding - orxFLOAT_1) /
                                        (iX1 - iX0)))
                              : pstLoadInfo->vFontScale.fY;
                      ...
                      /* Gets base line */
                      fBaseLine = orxMath_Ceil(pstLoadInfo->vFontScale.fY *
                                               orxS2F(iY1));

                      /* Stores size & spacing */
                      orxVector_Copy(&(pstLoadInfo->vCharacterSize),
                                     _pvCharacterSize);
                      orxVector_Copy(&(pstLoadInfo->vCharacterSpacing),
                                     _pvCharacterSpacing);
                      ...
                      for (u32CharacterCodePoint =
                               orxString_GetFirstCharacterCodePoint(
                                   _zCharacterList, &zCharacterList),
                          i = 0,
                          fCurrentWidth = fWidth = _pvCharacterSpacing->fX,
                          fHeight = pstLoadInfo->vCharacterSize.fY +
                                    orx2F(2.0f) * _pvCharacterSpacing->fY;
                           (u32CharacterCodePoint != orxCHAR_NULL) &&
                           (u32CharacterCodePoint != orxU32_UNDEFINED);
                           u32CharacterCodePoint =
                               orxString_GetFirstCharacterCodePoint(
                        ...
                        stbtt_GetGlyphHMetrics(
                            &(pstLoadInfo->stFontInfo),
                            pstLoadInfo->astGlyphList[i].s32Index, &iGlyphWidth,
                            NULL);
                        ...
                        stbtt_GetGlyphBitmapBox(
                            ...
                            pstLoadInfo->astGlyphList[i].s32Index,
                            pstLoadInfo->vFontScale.fX,
                            pstLoadInfo->vFontScale.fY, (int *)&iGlyphX0,
                            (int *)&iGlyphY0, (int *)&iGlyphX1,
                            (int *)&iGlyphY1);

                        /* Updates glyph values */
                        pstLoadInfo->astGlyphList[i].stGlyph.fWidth =
                            orx2F(2.0f) * fXPadding +
                            orxMath_Ceil(
                                (_pvCharacterSize->fX > orxFLOAT_0)
                                    ? _pvCharacterSize->fX
                                : (_pvCharacterSize->fX == orxFLOAT_0)
                                    ? orxMAX(pstLoadInfo->vFontScale.fX *
                                                 orxS2F(iGlyphWidth),
                                             orxS2F(iGlyphX1 - iGlyphX0))
                                    : pstLoadInfo->vFontScale.fX * (iX1 - iX0));
                        pstLoadInfo->astGlyphList[i].stGlyph.fX =
                            fXPadding +
                            ((_pvCharacterSize->fX == orxFLOAT_0)
                                 ? orxMAX(0, orxS2F(iGlyphX0))
                                 : orxMath_Floor(
                                       orx2F(0.5f) *
                                       (pstLoadInfo->astGlyphList[i]
                                            .stGlyph.fWidth -
                                        orx2F(2.0f) * fXPadding -
                                        orxS2F(iGlyphX1 - iGlyphX0))));
                        if (pstLoadInfo->bSDF != orxFALSE) {
                          ...
                          pstLoadInfo->astGlyphList[i].stGlyph.fY =
                              fYPadding -
                              pstLoadInfo->vFontScale.fY * orxS2F(iY0);
                        } else {
                          pstLoadInfo->astGlyphList[i].stGlyph.fY =
                              fYPadding + fBaseLine + orxS2F(iGlyphY0);
                        }

                        /* Gets horizontal advance */
                        fAdvance = pstLoadInfo->astGlyphList[i].stGlyph.fWidth +
                                   _pvCharacterSpacing->fX;
                        ...
                        if (fCurrentWidth + fAdvance <= fMaxTextureWidth) {
                          ...
                          fCurrentWidth =
                              pstLoadInfo->astGlyphList[i].stGlyph.fWidth +
                              orx2F(2.0f) * _pvCharacterSpacing->fX;
                          fHeight +=
                              _pvCharacterSize->fY + _pvCharacterSpacing->fY;
                        }
                        fWidth = orxMAX(fWidth, fCurrentWidth);

                        /* Updates character width list */
                        _afCharacterWidthList[i] =
                            pstLoadInfo->astGlyphList[i].stGlyph.fWidth;
                      }

                      /* Stores bitmap */
                      pstLoadInfo->stLoadInfo.pstBitmap =
                          (orxBITMAP *)pstResult;
                      ...
                      if (sstDisplay.pstTempBitmap != orxNULL) {
                        /* Inits bitmap info using temp */
                        pstResult->uiTexture =
                            sstDisplay.pstTempBitmap->uiTexture;
                        pstResult->fBorderFix =
                            sstDisplay.pstTempBitmap->fBorderFix;
                        pstResult->u32RealWidth =
                            sstDisplay.pstTempBitmap->u32RealWidth;
                        pstResult->u32RealHeight =
                            sstDisplay.pstTempBitmap->u32RealHeight;
                        pstResult->u32Depth =
                            sstDisplay.pstTempBitmap->u32Depth;
                        pstResult->fRecRealWidth =
                            sstDisplay.pstTempBitmap->fRecRealWidth;
                        pstResult->fRecRealHeight =
                            sstDisplay.pstTempBitmap->fRecRealHeight;
                        pstResult->u32DataSize =
                            sstDisplay.pstTempBitmap->u32DataSize;
                        orxVector_Copy(&(pstResult->stClip.vTL),
                                       &(sstDisplay.pstTempBitmap->stClip.vTL));
                        orxVector_Copy(&(pstResult->stClip.vBR),
                                       &(sstDisplay.pstTempBitmap->stClip.vBR));
                        orxFLAG_SET(pstResult->u32Flags,
                                    orxDISPLAY_KU32_BITMAP_FLAG_LOADING,
                                    orxDISPLAY_KU32_BITMAP_FLAG_NONE);

                        /* Runs asynchronous task */
                        if (orxThread_RunTask(
                                &orxDisplay_GLFW_ProcessFont,
                                orxDisplay_GLFW_DecompressBitmapCallback,
                                orxNULL,
                                (void *)pstLoadInfo) == orxSTATUS_FAILURE) {
                          /* Deletes glyph list */
                          orxMemory_Free(pstLoadInfo->astGlyphList);
                        ...
                        /* Processes font */
                        if (orxDisplay_GLFW_ProcessFont(pstLoadInfo) !=
                            orxSTATUS_FAILURE) {
                          /* Uploads texture */
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_GetBitmapSize(const orxBITMAP *_pstBitmap,
                                                    orxFLOAT *_pfWidth,
                                                    orxFLOAT *_pfHeight) {
  orxSTATUS eResult = orxSTATUS_SUCCESS;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_GetScreenSize(orxFLOAT *_pfWidth,
                                                    orxFLOAT *_pfHeight) {
  orxSTATUS eResult = orxSTATUS_SUCCESS;
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetBitmapClipping(orxBITMAP *_pstBitmap,
                                                        orxU32 _u32TLX,
                                                        orxU32 _u32TLY,
                                                        orxU32 _u32BRX,
                                                        orxU32 _u32BRY) {
  ...
  if (_pstBitmap == sstDisplay.apstDestinationBitmapList[0]) {
    ...
    if (_pstBitmap == sstDisplay.pstScreen) {
      ...
      u32ClipX =
          orxF2U(orxMath_Round(sstDisplay.vContentScale.fX * orxU2F(_u32TLX)));
      u32ClipY = orxF2U(
          orxMath_Round(sstDisplay.vContentScale.fY *
                        (sstDisplay.apstDestinationBitmapList[0]->fHeight -
                         orxU2F(_u32BRY))));
      u32ClipWidth = orxF2U(orxMath_Round(sstDisplay.vContentScale.fX *
                                          orxU2F(_u32BRX - _u32TLX)));
      u32ClipHeight = orxF2U(orxMath_Round(sstDisplay.vContentScale.fY *
                                           orxU2F(_u32BRY - _u32TLY)));
    } else {
      /* Gets new clipping values */
      u32ClipX = _u32TLX;
      u32ClipY = _u32TLY;
    ...
    if ((u32ClipX != sstDisplay.u32LastClipX) ||
      ...
      /* Sets OpenGL clipping */
      glScissor((GLint)u32ClipX, (GLint)u32ClipY, (GLsizei)u32ClipWidth,
                (GLsizei)u32ClipHeight);
      glASSERT();
  ...
  /* Stores clip coords */
  orxVector_Set(&(_pstBitmap->stClip.vTL), orxU2F(_u32TLX), orxU2F(_u32TLY),
                orxFLOAT_0);
  orxVector_Set(&(_pstBitmap->stClip.vBR), orxU2F(_u32BRX), orxU2F(_u32BRY),
                orxFLOAT_0);
...
orxDISPLAY_VIDEO_MODE *orxFASTCALL orxDisplay_GLFW_GetVideoMode(
  ...
  if (pstMonitor != orxNULL) {
    ...
    /* Is index valid? */
      ...
      _pstVideoMode->u32Depth = astModeList[_u32Index].redBits +
                                astModeList[_u32Index].greenBits +
                                astModeList[_u32Index].blueBits;
      _pstVideoMode->u32RefreshRate = (astModeList[_u32Index].refreshRate != 0)
                                          ? astModeList[_u32Index].refreshRate
                                          : sstDisplay.u32DefaultRefreshRate;
      _pstVideoMode->bFullScreen =
          orxFLAG_TEST(sstDisplay.u32Flags,
                       orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN)
...
orxBOOL orxFASTCALL orxDisplay_GLFW_IsVideoModeAvailable(
  ...
  if (pstMonitor != orxNULL) {
    ...
    for (i = 0; i < s32Count; i++) {
      ...
      if ((_pstVideoMode->u32Width == (orxU32)astModeList[i].width) &&
          ...
          ((_pstVideoMode->u32Depth ==
            (orxU32)(astModeList[i].redBits + astModeList[i].greenBits +
                     astModeList[i].blueBits)) ||
           ((_pstVideoMode->u32Depth == 32) &&
            (astModeList[i].redBits + astModeList[i].greenBits +
                 astModeList[i].blueBits ==
             24))) &&
          ((_pstVideoMode->u32RefreshRate ==
            (orxU32)astModeList[i].refreshRate) ||
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_EnableVSync(orxBOOL _bEnable) {
  ...
  if (_bEnable != orxFALSE) {
    /* Has VSync fix already happened? (to prevent busy loop in some graphics
     * drivers) */
    if (orxFLAG_TEST(sstDisplay.u32Flags,
                     orxDISPLAY_KU32_STATIC_FLAG_VSYNC_FIX)) {
    ...
    /* Updates status */
    orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VSYNC,
                orxDISPLAY_KU32_STATIC_FLAG_NONE);
  } else {
    /* Updates VSync status */
    glfwSwapInterval(0);
...
orxBOOL orxFASTCALL orxDisplay_GLFW_IsVSyncEnabled() {
  ...
  /* Updates result */
  bResult = orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VSYNC)
                ? orxTRUE
                : orxFALSE;
...
orxDisplay_GLFW_SetVideoMode(const orxDISPLAY_VIDEO_MODE *_pstVideoMode) {
  ...
  if (_pstVideoMode != orxNULL) {
    ...
    /* Gets its info */
    iWidth = (int)((_pstVideoMode->u32Width != 0) ? _pstVideoMode->u32Width
                                                  : sstDisplay.u32DefaultWidth);
    iHeight =
        (int)((_pstVideoMode->u32Height != 0) ? _pstVideoMode->u32Height
                                              : sstDisplay.u32DefaultHeight);
    iDepth = (int)((_pstVideoMode->u32Depth != 0) ? _pstVideoMode->u32Depth
                                                  : sstDisplay.u32DefaultDepth);
    iRefreshRate = (int)(((_pstVideoMode->bFullScreen != orxFALSE) &&
                          (_pstVideoMode->u32RefreshRate != 0))
                             ? _pstVideoMode->u32RefreshRate
                             : sstDisplay.u32DefaultRefreshRate);
    ...
    if (orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_ALLOW_RESIZE) == orxFALSE) {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NO_RESIZE,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    } else
#endif /* __orxWEB__ */
    {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
                  orxDISPLAY_KU32_STATIC_FLAG_NO_RESIZE);
    }
    ...
    if ((orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_DECORATION) != orxFALSE) &&
      ...
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags,
                  orxDISPLAY_KU32_STATIC_FLAG_NO_DECORATION,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    } else {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
    ...
    if (orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_DEPTHBUFFER) != orxFALSE) {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_DEPTHBUFFER,
                  orxDISPLAY_KU32_STATIC_FLAG_NONE);
    } else {
      /* Updates flags */
      orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_NONE,
    ...
    if ((sstDisplay.pstWindow != orxNULL) &&
        ...
        (orxFLAG_TEST(sstDisplay.u32Flags,
                      orxDISPLAY_KU32_STATIC_FLAG_HIDDEN) !=
         glfwGetWindowAttrib(sstDisplay.pstWindow, GLFW_VISIBLE))) {
      /* Ignores resize event for now */
      sstDisplay.u32Flags |= orxDISPLAY_KU32_STATIC_FLAG_IGNORE_EVENT;

      /* Updates window attributes */
      glfwSetWindowAttrib(sstDisplay.pstWindow, GLFW_RESIZABLE,
                          orxFLAG_TEST(sstDisplay.u32Flags,
                                       orxDISPLAY_KU32_STATIC_FLAG_NO_RESIZE)
                              ? GLFW_FALSE
                              : GLFW_TRUE);
      glfwSetWindowAttrib(
          sstDisplay.pstWindow, GLFW_DECORATED,
          orxFLAG_TEST(sstDisplay.u32Flags,
                       orxDISPLAY_KU32_STATIC_FLAG_NO_DECORATION)
              ? GLFW_FALSE
              : GLFW_TRUE);
      ...
      glfwSetWindowMonitor(
          sstDisplay.pstWindow,
          (_pstVideoMode->bFullScreen != orxFALSE) ? pstMonitor : orxNULL,
          (int)sstDisplay.vWindowPosition.fX,
          (int)sstDisplay.vWindowPosition.fY, iWidth, iHeight, iRefreshRate);
      ...
      glfwWindowHint(
          GLFW_VISIBLE,
          orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_HIDDEN)
              ? GLFW_FALSE
              : GLFW_TRUE);
      glfwWindowHint(GLFW_SAMPLES, 0);

      /* Creates window */
      pstNewWindow = glfwCreateWindow(
          iWidth, iHeight, orxConfig_GetString(orxDISPLAY_KZ_CONFIG_TITLE),
          (_pstVideoMode->bFullScreen != orxFALSE) ? pstMonitor : orxNULL,
          sstDisplay.pstWindow);
      ...
      if (pstNewWindow != orxNULL) {
        ...
        if (orxFLAG_TEST(sstDisplay.u32Flags,
          ...
          static const orxSTRING szFragmentShaderSource =
              ...
              "void main()"
              "{"
              "  gl_FragColor = _Color0_.rgba * texture2D(orxTexture, "
              "_gl_TexCoord0_).rgba;"
              "}";
          ...
          if (orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_SHADER_VERSION)) {
            /* Stores it */
            u32ShaderVersion =
                orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_SHADER_VERSION);
          ...
          sstDisplay.pstDefaultShader =
              (orxDISPLAY_SHADER *)orxDisplay_CreateShader(
                  &szFragmentShaderSource, 1, orxNULL, orxFALSE);
          sstDisplay.pstNoTextureShader =
              (orxDISPLAY_SHADER *)orxDisplay_CreateShader(
                  &szNoTextureFragmentShaderSource, 1, orxNULL, orxTRUE);

          /* Should restore shader version? */
          if (u32ShaderVersion != orxU32_UNDEFINED) {
            /* Restores it */
            orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_SHADER_VERSION,
                             u32ShaderVersion);
          }
        ...
        if (bFirstWindow != orxFALSE) {
          ...
          if (orxFLAG_TEST(sstDisplay.u32Flags,
            ...
            /* Fills IBO */
            glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
                            orxDISPLAY_KU32_INDEX_BUFFER_SIZE *
                                sizeof(GLushort),
                            sstDisplay.au16IndexList, GL_STATIC_DRAW_ARB);
            glASSERT();
          }
    ...
    if (eResult != orxSTATUS_FAILURE) {
      ...
      /* Gets framebuffer size */
      glfwGetFramebufferSize(sstDisplay.pstWindow, &iFramebufferWidth,
                             &iFramebufferHeight);

      /* Sets content scale */
      orxVector_Set(&(sstDisplay.vContentScale),
                    orxS2F(iFramebufferWidth) / orxS2F(iWidth),
                    orxS2F(iFramebufferHeight) / orxS2F(iHeight), orxFLOAT_1);

#ifndef __orxWEB__
      /* Is fullscreen? */
      if (_pstVideoMode->bFullScreen != orxFALSE) {
        /* Updates status */
        orxFLAG_SET(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_FULLSCREEN,
                    orxDISPLAY_KU32_STATIC_FLAG_NONE);
      } else
#endif /* !__orxWEB__ */
      {
      ...
      sstDisplay.pstScreen->fRecRealWidth =
          orxFLOAT_1 / orxU2F(sstDisplay.pstScreen->u32RealWidth);
      sstDisplay.pstScreen->fRecRealHeight =
          orxFLOAT_1 / orxU2F(sstDisplay.pstScreen->u32RealHeight);
      sstDisplay.pstScreen->u32DataSize = sstDisplay.pstScreen->u32RealWidth *
                                          sstDisplay.pstScreen->u32RealHeight *
                                          4 * sizeof(orxU8);
      sstDisplay.pstScreen->u32Flags =
          (sstDisplay.bDefaultSmoothing != orxFALSE)
              ? orxDISPLAY_KU32_BITMAP_FLAG_SMOOTHING
              : orxDISPLAY_KU32_BITMAP_FLAG_NONE;
      orxVector_Copy(&(sstDisplay.pstScreen->stClip.vTL), &orxVECTOR_0);
      orxVector_Set(&(sstDisplay.pstScreen->stClip.vBR),
                    sstDisplay.pstScreen->fWidth, sstDisplay.pstScreen->fHeight,
                    orxFLOAT_0);
      ...
      u32ClipWidth = orxF2U(orxMath_Round(sstDisplay.pstScreen->fWidth *
                                          sstDisplay.vContentScale.fX));
      u32ClipHeight = orxF2U(orxMath_Round(sstDisplay.pstScreen->fHeight *
                                           sstDisplay.vContentScale.fY));

      /* Clears new display surface */
      glScissor(0, 0, (GLsizei)u32ClipWidth, (GLsizei)u32ClipHeight);
      glASSERT();
      glClearColor(
          orxCOLOR_NORMALIZER * orxU2F(orxRGBA_R(sstDisplay.stLastColor)),
          orxCOLOR_NORMALIZER * orxU2F(orxRGBA_G(sstDisplay.stLastColor)),
          orxCOLOR_NORMALIZER * orxU2F(orxRGBA_B(sstDisplay.stLastColor)),
          orxCOLOR_NORMALIZER * orxU2F(orxRGBA_A(sstDisplay.stLastColor)));
      glASSERT();
      glClear(GL_COLOR_BUFFER_BIT);
      glASSERT();
      ...
      /* Sends event */
      orxEVENT_SEND(orxEVENT_TYPE_DISPLAY, orxDISPLAY_EVENT_SET_VIDEO_MODE,
                    orxNULL, orxNULL, &stPayload);
    }
  }
  ...
  if (eResult != orxSTATUS_FAILURE) {
    ...
    if (!orxFLAG_TEST(sstDisplay.u32Flags,
      ...
      if (pstMonitor != orxNULL) {
        ...
        if (glfwGetError(NULL) == GLFW_NO_ERROR) {
          ...
          orxConfig_GetVector(orxDISPLAY_KZ_CONFIG_POSITION,
                              &(sstDisplay.vWindowPosition));
          ...
          glfwSetWindowPos(sstDisplay.pstWindow,
                           (int)sstDisplay.vWindowPosition.fX,
                           (int)sstDisplay.vWindowPosition.fY);
        }
      }
    }
#endif /* !__orxWEB__ */
    ...
    /* Updates its title */
    glfwSetWindowTitle(sstDisplay.pstWindow,
                       orxConfig_GetString(orxDISPLAY_KZ_CONFIG_TITLE));
    ...
    sstDisplay.fLastOrthoRight =
        (GLfloat)(sstDisplay.apstDestinationBitmapList[0] != orxNULL)
            ? sstDisplay.apstDestinationBitmapList[0]->fWidth
            : sstDisplay.pstScreen->fWidth;
    sstDisplay.fLastOrthoBottom =
        (GLfloat)(sstDisplay.apstDestinationBitmapList[0] != orxNULL)
            ? (sstDisplay.apstDestinationBitmapList[0] == sstDisplay.pstScreen)
                  ? sstDisplay.apstDestinationBitmapList[0]->fHeight
                  : -sstDisplay.apstDestinationBitmapList[0]->fHeight
    ...
    /* Has VBO support? */
    if (orxFLAG_TEST_ALL(sstDisplay.u32Flags,
                         orxDISPLAY_KU32_STATIC_FLAG_VBO |
                             orxDISPLAY_KU32_STATIC_FLAG_SHADER)) {
      /* Inits projection matrix */
      (sstDisplay.fLastOrthoBottom >= 0.0)
          ? orxDisplay_GLFW_OrthoProjMatrix(
                &(sstDisplay.mProjectionMatrix), orxFLOAT_0,
                sstDisplay.fLastOrthoRight, sstDisplay.fLastOrthoBottom,
                orxFLOAT_0, -orxFLOAT_1, orxFLOAT_1)
          : orxDisplay_GLFW_OrthoProjMatrix(
                &(sstDisplay.mProjectionMatrix), orxFLOAT_0,
                sstDisplay.fLastOrthoRight, orxFLOAT_0,
                -sstDisplay.fLastOrthoBottom, -orxFLOAT_1, orxFLOAT_1);

      /* Passes it to shader */
      glUNIFORM(Matrix4fvARB,
                sstDisplay.pstDefaultShader->iProjectionMatrixLocation, 1,
                GL_FALSE,
                (GLfloat *)&(sstDisplay.mProjectionMatrix.aafValueList[0][0]));
      ...
      glVertexAttribPointerARB(orxDISPLAY_ATTRIBUTE_LOCATION_VERTEX, 2,
                               GL_FLOAT, GL_FALSE, sizeof(orxDISPLAY_VERTEX),
                               (GLvoid *)offsetof(orxDISPLAY_GLFW_VERTEX, fX));
      glASSERT();
      glVertexAttribPointerARB(orxDISPLAY_ATTRIBUTE_LOCATION_TEXCOORD, 2,
                               GL_FLOAT, GL_FALSE, sizeof(orxDISPLAY_VERTEX),
                               (GLvoid *)offsetof(orxDISPLAY_GLFW_VERTEX, fU));
      glASSERT();
      glVertexAttribPointerARB(
          orxDISPLAY_ATTRIBUTE_LOCATION_COLOR, 4, GL_UNSIGNED_BYTE, GL_FALSE,
          sizeof(orxDISPLAY_VERTEX),
          (GLvoid *)offsetof(orxDISPLAY_GLFW_VERTEX, stRGBA));
      glASSERT();
    }
#ifndef __orxDISPLAY_OPENGL_ES__
      ...
      glASSERT();
      (sstDisplay.fLastOrthoBottom >= 0.0)
          ? glOrtho(0.0f, (GLdouble)sstDisplay.fLastOrthoRight,
                    (GLdouble)sstDisplay.fLastOrthoBottom, 0.0f, -1.0f, 1.0f)
          : glOrtho(0.0f, (GLdouble)sstDisplay.fLastOrthoRight, 0.0f,
                    -(GLdouble)sstDisplay.fLastOrthoBottom, -1.0f, 1.0f);
      glASSERT();
      ...
      /* Selects arrays */
      glVertexPointer(
          2, GL_FLOAT, sizeof(orxDISPLAY_VERTEX),
          orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO)
              ? (GLvoid *)offsetof(orxDISPLAY_GLFW_VERTEX, fX)
              : &(sstDisplay.astVertexList[0].fX));
      glASSERT();
      glTexCoordPointer(
      ...
      glASSERT();
      glColorPointer(
          4, GL_UNSIGNED_BYTE, sizeof(orxDISPLAY_VERTEX),
          orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_VBO)
              ? (GLvoid *)offsetof(orxDISPLAY_GLFW_VERTEX, stRGBA)
              : &(sstDisplay.astVertexList[0].stRGBA));
      glASSERT();
    ...
    glfwGetFramebufferSize(sstDisplay.pstWindow, &iFramebufferWidth,
                           &iFramebufferHeight);
    orxVector_Set(&vFramebufferSize, orx2F(iFramebufferWidth),
                  orx2F(iFramebufferHeight), orxFLOAT_0);
    ...
    orxConfig_SetFloat(orxDISPLAY_KZ_CONFIG_HEIGHT,
                       sstDisplay.pstScreen->fHeight);
    orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_DEPTH,
                     sstDisplay.pstScreen->u32Depth);
    orxConfig_SetU32(orxDISPLAY_KZ_CONFIG_REFRESH_RATE,
                     sstDisplay.u32RefreshRate);
    orxConfig_SetVector(orxDISPLAY_KZ_CONFIG_FRAMEBUFFER_SIZE,
                        &vFramebufferSize);
    orxConfig_SetVector(orxDISPLAY_KZ_CONFIG_CONTENT_SCALE,
                        &(sstDisplay.vContentScale));
  }
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_Init() {
  ...
  if (!(sstDisplay.u32Flags & orxDISPLAY_KU32_STATIC_FLAG_READY)) {
    ...
    /* For all indices */
    for (i = 0, u16Index = 0; i < orxDISPLAY_KU32_INDEX_BUFFER_SIZE;
         i += 6, u16Index += 4) {
      /* Computes them */
    ...
    /* Inits GLFW */
    eResult =
        (glfwInit() != GLFW_FALSE) ? orxSTATUS_SUCCESS : orxSTATUS_FAILURE;

    /* Valid? */
    if (eResult != orxSTATUS_FAILURE) {
      /* Adds event handlers */
      eResult = orxEvent_AddHandler(orxEVENT_TYPE_RENDER,
                                    orxDisplay_GLFW_EventHandler);
      eResult = (eResult != orxSTATUS_FAILURE)
                    ? orxEvent_AddHandler(orxEVENT_TYPE_SYSTEM,
                                          orxDisplay_GLFW_EventHandler)
                    : orxSTATUS_FAILURE;
      eResult = (eResult != orxSTATUS_FAILURE)
                    ? orxEvent_AddHandler(orxEVENT_TYPE_FIRST_RESERVED,
                                          orxDisplay_GLFW_EventHandler)
                    : orxSTATUS_FAILURE;
      ...
      if (eResult != orxSTATUS_FAILURE) {
        ...
        orxEvent_SetHandlerIDFlags(
            orxDisplay_GLFW_EventHandler, orxEVENT_TYPE_RENDER, orxNULL,
            orxEVENT_GET_FLAG(orxRENDER_EVENT_STOP), orxEVENT_KU32_MASK_ID_ALL);
        orxEvent_SetHandlerIDFlags(orxDisplay_GLFW_EventHandler,
                                   orxEVENT_TYPE_SYSTEM, orxNULL,
                                   orxEVENT_GET_FLAG(orxSYSTEM_EVENT_CLIPBOARD),
                                   orxEVENT_KU32_MASK_ID_ALL);

        /* Creates banks */
        sstDisplay.pstBitmapBank =
            orxBank_Create(orxDISPLAY_KU32_BITMAP_BANK_SIZE, sizeof(orxBITMAP),
        ...
        if ((sstDisplay.pstBitmapBank != orxNULL) &&
          ...
          stVideoMode.u32Width =
              orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_WIDTH)
                  ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_WIDTH)
                  : sstDisplay.u32DefaultWidth;
          stVideoMode.u32Height =
              orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_HEIGHT)
                  ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_HEIGHT)
                  : sstDisplay.u32DefaultHeight;
          stVideoMode.u32Depth =
              orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_DEPTH)
                  ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_DEPTH)
                  : sstDisplay.u32DefaultDepth;
          stVideoMode.u32RefreshRate =
              orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_REFRESH_RATE)
                  ? orxConfig_GetU32(orxDISPLAY_KZ_CONFIG_REFRESH_RATE)
                  : sstDisplay.u32DefaultRefreshRate;
#ifdef __orxWEB__
...
#else  /* __orxWEB__ */
          stVideoMode.bFullScreen =
              orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_FULLSCREEN);
#endif /* __orxWEB__ */
          ...
          /* Has decoration? */
          if ((orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_DECORATION) ==
               orxFALSE) ||
              (orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_DECORATION) !=
               orxFALSE)) {
            /* Stores default decorated position */
            orxVector_Set(&(sstDisplay.vWindowPosition),
                          orxDISPLAY_KV_DEFAULT_DECORATED_POSITION);
          }

          /* Allocates screen bitmap */
          sstDisplay.pstScreen =
              (orxBITMAP *)orxBank_Allocate(sstDisplay.pstBitmapBank);
          orxMemory_Zero(sstDisplay.pstScreen, sizeof(orxBITMAP));

          /* Inits info */
          sstDisplay.bDefaultSmoothing =
              orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_SMOOTH);
          sstDisplay.eLastBlendMode = orxDISPLAY_BLEND_MODE_NUMBER;
          sstDisplay.eLastBufferMode = orxDISPLAY_BUFFER_MODE_NUMBER;
          sstDisplay.ePrimitive = orxDISPLAY_KE_DEFAULT_PRIMITIVE;

          /* Sets video mode? */
          if ((eResult = orxDisplay_GLFW_SetVideoMode(&stVideoMode)) ==
              orxSTATUS_FAILURE) {
            /* Updates resolution */
          ...
          if (eResult != orxSTATUS_FAILURE) {
            ...
            if (orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_VSYNC) != orxFALSE) {
              ...
              orxDisplay_GLFW_EnableVSync(
                  orxConfig_GetBool(orxDISPLAY_KZ_CONFIG_VSYNC));
            } else {
              /* Enables vertical sync */
              orxDisplay_GLFW_EnableVSync(orxTRUE);
            }

            /* Only allows delayed VSync to fix a busy loop issue in some
             * graphics drivers */
            orxClock_AddGlobalTimer(orxDisplay_GLFW_VSyncFix,
                                    orxDISPLAY_KF_VSYNC_DELAY_FIX, 1, orxNULL);
            ...
            if ((pstClock != orxNULL) &&
                ((eResult = orxClock_Register(pstClock, orxDisplay_GLFW_Update,
                                              orxNULL, orxMODULE_ID_DISPLAY,
              ...
              glfwSetWindowSizeCallback(sstDisplay.pstWindow,
                                        orxDisplay_GLFW_ResizeCallback);

              /* Registers content scale callback */
              glfwSetWindowContentScaleCallback(
                  sstDisplay.pstWindow, orxDisplay_GLFW_ContentScaleCallback);

              /* Registers drop callback */
              glfwSetDropCallback(sstDisplay.pstWindow,
                                  orxDisplay_GLFW_DropCallback);

              /* Registers position callback */
              glfwSetWindowPosCallback(sstDisplay.pstWindow,
                                       orxDisplay_GLFW_PosCallback);
              ...
              /* Forces a position update */
              orxDisplay_GLFW_PosCallback(sstDisplay.pstWindow, iWindowX,
                                          iWindowY);
            } else {
              /* Terminates GLFW */
              glfwTerminate();
              ...
              /* Logs message */
              orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                             "Failed to register GLFW/display event handler.");
            }
          } else {
            /* Terminates GLFW */
            glfwTerminate();
            ...
            /* Logs message */
            orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                           "Failed to init GLFW/OpenGL default video mode.");
          }
          ...
          /* Logs message */
          orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                         "Failed to create bitmap/shader banks.");
        }
      } else {
        /* Exits from GLFW */
        glfwTerminate();
        ...
        orxEvent_RemoveHandler(orxEVENT_TYPE_SYSTEM,
                               orxDisplay_GLFW_EventHandler);
        orxEvent_RemoveHandler(orxEVENT_TYPE_FIRST_RESERVED,
                               orxDisplay_GLFW_EventHandler);
...
orxBOOL orxFASTCALL orxDisplay_GLFW_HasShaderSupport() {
  ...
  /* Done! */
  return (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_SHADER))
             ? orxTRUE
             : orxFALSE;
}

orxHANDLE orxFASTCALL orxDisplay_GLFW_CreateShader(
  ...
  if (orxFLAG_TEST(sstDisplay.u32Flags, orxDISPLAY_KU32_STATIC_FLAG_SHADER)) {
    ...
    if ((_azCodeList != orxNULL) && (_u32Size > 0)) {
      ...
      pstShader =
          (orxDISPLAY_SHADER *)orxBank_Allocate(sstDisplay.pstShaderBank);
      ...
      if (pstShader != orxNULL) {
        ...
        sstDisplay.acShaderCodeBuffer[0] =
            sstDisplay
                .acShaderCodeBuffer[orxDISPLAY_KU32_SHADER_BUFFER_SIZE - 1] =
                orxCHAR_NULL;
        ...
        s32Offset = orxString_NPrint(
            pc, s32Free, "#extension GL_EXT_draw_buffers : enable\n");
        pc += s32Offset;
        s32Free -= s32Offset;
        ...
        if (orxConfig_HasValue(orxDISPLAY_KZ_CONFIG_SHADER_EXTENSION_LIST) !=
            orxFALSE) {
          orxS32 i, s32Count;

          /* For all extensions */
          for (i = 0, s32Count = orxConfig_GetListCount(
            ...
            /* Gets it */
            zExtension = orxConfig_GetListString(
                orxDISPLAY_KZ_CONFIG_SHADER_EXTENSION_LIST, i);
            ...
            /* Prints it */
            s32Offset = orxString_NPrint(
                pc, s32Free, "#extension %s : %s\n", zExtension,
                (bAdd != orxFALSE) ? "enable" : "disable");
            pc += s32Offset;
            s32Free -= s32Offset;
          }
        }
          ...
          s32Offset = orxString_NPrint(
              pc, s32Free, "#extension GL_EXT_gpu_shader4 : enable\n");
          pc += s32Offset;
          s32Free -= s32Offset;
        }
        ...
        if (_pstParamList != orxNULL) {
          ...
          /* For all parameters */
          for (pstParam =
                   (orxSHADER_PARAM *)orxLinkList_GetFirst(_pstParamList);
               pstParam != orxNULL;
               pstParam = (orxSHADER_PARAM *)orxLinkList_GetNext(
                   &(pstParam->stNode))) {
            /* Depending on type */
            ...
            case orxSHADER_PARAM_TYPE_FLOAT: {
              ...
              s32Offset =
                  (pstParam->u32ArraySize >= 1)
                      ? orxString_NPrint(pc, s32Free, "uniform float %s[%u];\n",
                                         pstParam->zName,
                                         pstParam->u32ArraySize)
                      : orxString_NPrint(pc, s32Free, "uniform float %s;\n",
                                         pstParam->zName);
              pc += s32Offset;
              s32Free -= s32Offset;
            ...
            case orxSHADER_PARAM_TYPE_TEXTURE: {
              ...
              s32Offset =
                  (pstParam->u32ArraySize >= 1)
                      ? orxString_NPrint(
                            pc, s32Free,
                            "uniform sampler2D %s[%u];\nuniform float "
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_TOP
                            "[%u];\nuniform float "
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_LEFT
                            "[%u];\nuniform float "
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_BOTTOM
                            "[%u];\nuniform float "
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_RIGHT "[%u];\n",
                            pstParam->zName, pstParam->u32ArraySize,
                            pstParam->zName, pstParam->u32ArraySize,
                            pstParam->zName, pstParam->u32ArraySize,
                            pstParam->zName, pstParam->u32ArraySize,
                            pstParam->zName, pstParam->u32ArraySize)
                      : orxString_NPrint(
                            ...
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_BOTTOM
                            ";\nuniform float "
                            "%s" orxDISPLAY_KZ_SHADER_SUFFIX_RIGHT ";\n",
                            pstParam->zName, pstParam->zName, pstParam->zName,
            ...
            case orxSHADER_PARAM_TYPE_VECTOR: {
              ...
              s32Offset =
                  (pstParam->u32ArraySize >= 1)
                      ? orxString_NPrint(pc, s32Free, "uniform vec3 %s[%u];\n",
                                         pstParam->zName,
                                         pstParam->u32ArraySize)
                      : orxString_NPrint(pc, s32Free, "uniform vec3 %s;\n",
                                         pstParam->zName);
              pc += s32Offset;
              s32Free -= s32Offset;
        ...
        for (pcReplace = (orxCHAR *)orxString_SearchString(
                 sstDisplay.acShaderCodeBuffer, "gl_TexCoord[0]");
             pcReplace != orxNULL;
             pcReplace = (orxCHAR *)orxString_SearchString(
                 pcReplace + 14 * sizeof(orxCHAR), "gl_TexCoord[0]")) {
          /* Replaces it */
        ...
        /* For all gl_Color */
        for (pcReplace = (orxCHAR *)orxString_SearchString(
                 sstDisplay.acShaderCodeBuffer, "gl_Color");
             pcReplace != orxNULL;
             pcReplace = (orxCHAR *)orxString_SearchString(
                 pcReplace + 8 * sizeof(orxCHAR), "gl_Color")) {
          /* Replaces it */
        ...
        pstShader->astTextureInfoList =
            (orxDISPLAY_TEXTURE_INFO *)orxMemory_Allocate(
                ...
                orxMEMORY_TYPE_MAIN);
        pstShader->astParamInfoList =
            (orxDISPLAY_PARAM_INFO *)orxMemory_Allocate(
                sstDisplay.iTextureUnitNumber * sizeof(orxDISPLAY_PARAM_INFO),
                orxMEMORY_TYPE_MAIN);
        orxMemory_Zero(pstShader->astTextureInfoList,
                       sstDisplay.iTextureUnitNumber *
                           sizeof(orxDISPLAY_TEXTURE_INFO));
        orxMemory_Zero(pstShader->astParamInfoList,
                       sstDisplay.iTextureUnitNumber *
                           sizeof(orxDISPLAY_PARAM_INFO));
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_StartShader(orxHANDLE _hShader) {
  ...
  glUNIFORM(Matrix4fvARB, pstShader->iProjectionMatrixLocation, 1, GL_FALSE,
            (GLfloat *)&(sstDisplay.mProjectionMatrix.aafValueList[0][0]));
  ...
  if (pstShader != orxNULL) {
    ...
    if ((orxLinkList_GetList(&(pstShader->stNode)) != orxNULL) &&
      ...
      if (sstDisplay.s32BufferIndex == 0) {
        ...
        /* For all active shaders */
        for (pstActive = (orxDISPLAY_SHADER *)orxLinkList_GetFirst(
                 &(sstDisplay.stActiveShaderList));
             pstActive != orxNULL; pstActive = pstNextActive) {
          /* Gets next shader */
          pstNextActive =
              (orxDISPLAY_SHADER *)orxLinkList_GetNext(&(pstActive->stNode));
          ...
          if (pstActive->bPending != orxFALSE) {
            ...
            /* Clears its texture info list */
            orxMemory_Zero(pstActive->astTextureInfoList,
                           sstDisplay.iTextureUnitNumber *
                               sizeof(orxDISPLAY_TEXTURE_INFO));
...
orxS32 orxFASTCALL orxDisplay_GLFW_GetParameterID(const orxHANDLE _hShader,
  ...
  if (_bIsTexture != orxFALSE) {
    ...
    if (_s32Index >= 0) {
      /* Prints its name */
      orxString_NPrint(acBuffer, sizeof(acBuffer), "%s[%d]", _zParam,
                       _s32Index);

      /* Gets parameter location */
      pstInfo->iLocation =
          glGetUniformLocationARB(pstShader->hProgram, acBuffer);
      glASSERT();

      /* Gets top parameter location */
      orxString_NPrint(acBuffer, sizeof(acBuffer),
                       "%s" orxDISPLAY_KZ_SHADER_SUFFIX_TOP "[%d]", _zParam,
                       _s32Index);
      pstInfo->iLocationTop = glGetUniformLocationARB(pstShader->hProgram,
                                                      (const GLchar *)acBuffer);
      glASSERT();

      /* Gets left parameter location */
      orxString_NPrint(acBuffer, sizeof(acBuffer),
                       "%s" orxDISPLAY_KZ_SHADER_SUFFIX_LEFT "[%d]", _zParam,
                       _s32Index);
      pstInfo->iLocationLeft = glGetUniformLocationARB(
          pstShader->hProgram, (const GLchar *)acBuffer);
      glASSERT();

      /* Gets bottom parameter location */
      orxString_NPrint(acBuffer, sizeof(acBuffer),
                       "%s" orxDISPLAY_KZ_SHADER_SUFFIX_BOTTOM "[%d]", _zParam,
                       _s32Index);
      pstInfo->iLocationBottom = glGetUniformLocationARB(
          pstShader->hProgram, (const GLchar *)acBuffer);
      glASSERT();

      /* Gets right parameter location */
      orxString_NPrint(acBuffer, sizeof(acBuffer),
                       "%s" orxDISPLAY_KZ_SHADER_SUFFIX_RIGHT "[%d]", _zParam,
                       _s32Index);
      pstInfo->iLocationRight = glGetUniformLocationARB(
          pstShader->hProgram, (const GLchar *)acBuffer);
      glASSERT();
    } else {
      /* Gets parameter location */
      pstInfo->iLocation =
          glGetUniformLocationARB(pstShader->hProgram, (const GLchar *)_zParam);
      glASSERT();
    ...
    if (pstShader->bUseCustomParam == orxFALSE) {
      ...
      if ((pstInfo->iLocationTop >= 0) || (pstInfo->iLocationLeft >= 0) ||
          (pstInfo->iLocationBottom >= 0) || (pstInfo->iLocationRight >= 0)) {
        /* Updates status */
        ...
        orxDEBUG_PRINT(
            orxDEBUG_LEVEL_DISPLAY,
            "Shader [%u] with \"UseCustomParam = false\" is using edge "
            "parameter for texture [%s]: forcing UseCustomParam to true.",
            (orxU32)(orxUPTR)pstShader->hProgram, _zParam);
      }
    }
  } else {
    /* Array? */
    if (_s32Index >= 0) {
      ...
      /* Gets parameter location */
      s32Result =
          (orxS32)glGetUniformLocationARB(pstShader->hProgram, acBuffer);
      glASSERT();
    } else {
      /* Gets parameter location */
      s32Result = (orxS32)glGetUniformLocationARB(pstShader->hProgram,
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetShaderBitmap(
  ...
  orxASSERT((sstDisplay.u32Flags & orxDISPLAY_KU32_STATIC_FLAG_READY) ==
            orxDISPLAY_KU32_STATIC_FLAG_READY);
  orxASSERT((_hShader != orxHANDLE_UNDEFINED) && (_hShader != orxNULL));
  orxASSERT((_pstValue != sstDisplay.pstScreen) &&
                "Can't use screen bitmap as texture parameter (ID <%d>) for "
                "fragment shader.",
            _s32ID);
  ...
  for (i = 0; i < pstShader->iTextureCount; i++) {
    /* Same location? */
    if (pstShader->astTextureInfoList[i].iLocation ==
        pstShader->astParamInfoList[_s32ID].iLocation) {
      /* Different texture? */
      if (pstShader->astTextureInfoList[i].pstBitmap != _pstValue) {
        ...
        glUNIFORM(
            1fARB, pstShader->astParamInfoList[_s32ID].iLocationTop,
            (GLfloat)(_pstValue->fRecRealHeight * _pstValue->stClip.vTL.fY));
        glUNIFORM(
            1fARB, pstShader->astParamInfoList[_s32ID].iLocationLeft,
            (GLfloat)(_pstValue->fRecRealWidth * _pstValue->stClip.vTL.fX));
        glUNIFORM(
            1fARB, pstShader->astParamInfoList[_s32ID].iLocationBottom,
            (GLfloat)(_pstValue->fRecRealHeight * _pstValue->stClip.vBR.fY));
        glUNIFORM(
            1fARB, pstShader->astParamInfoList[_s32ID].iLocationRight,
            (GLfloat)(_pstValue->fRecRealWidth * _pstValue->stClip.vBR.fX));
      }
  ...
  if (eResult == orxSTATUS_FAILURE) {
    ...
    if (pstShader->iTextureCount < sstDisplay.iTextureUnitNumber) {
      ...
      if (_s32ID >= 0) {
        /* Updates texture info */
        pstShader->astTextureInfoList[pstShader->iTextureCount].iLocation =
            pstShader->astParamInfoList[_s32ID].iLocation;
        pstShader->astTextureInfoList[pstShader->iTextureCount].pstBitmap =
            _pstValue;
        ...
        /* Outputs log */
        orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                       "Can't bind texture parameter (ID <%d>) for fragment "
                       "shader: invalid ID.",
                       _s32ID);
      }
    } else {
      ...
      orxDEBUG_PRINT(orxDEBUG_LEVEL_DISPLAY,
                     "Can't bind texture parameter (ID <%d>) for fragment "
                     "shader: all the texture units are used.",
                     _s32ID);
    }
  }
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetShaderFloat(orxHANDLE _hShader,
                                                     orxS32 _s32ID,
                                                     orxFLOAT _fValue) {
  orxSTATUS eResult;
  ...
  /* Valid? */
  if (_s32ID >= 0) {
    /* Updates its value (no glASSERT() as this can be set more than once per
     * use and would trigger it) */
...
orxSTATUS orxFASTCALL orxDisplay_GLFW_SetShaderVector(
  ...
  if (_s32ID >= 0) {
    /* Updates its value (no glASSERT() as this can be set more than once per
     * use and would trigger it) */
    glUNIFORM_NO_ASSERT(3fARB, (GLint)_s32ID, (GLfloat)_pvValue->fX,
                        (GLfloat)_pvValue->fY, (GLfloat)_pvValue->fZ);
...
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetScreenBitmap, DISPLAY,
                                 GET_SCREEN_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetScreenSize, DISPLAY,
                                 GET_SCREEN_SIZE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_CreateBitmap, DISPLAY,
                                 CREATE_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DeleteBitmap, DISPLAY,
                                 DELETE_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_LoadBitmap, DISPLAY,
                                 LOAD_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SaveBitmap, DISPLAY,
                                 SAVE_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_LoadFont, DISPLAY, LOAD_FONT);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetTempBitmap, DISPLAY,
                                 SET_TEMP_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetTempBitmap, DISPLAY,
                                 GET_TEMP_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetDestinationBitmaps, DISPLAY,
                                 SET_DESTINATION_BITMAPS);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_ClearBitmap, DISPLAY,
                                 CLEAR_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetBlendMode, DISPLAY,
                                 SET_BLEND_MODE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetBitmapClipping, DISPLAY,
                                 SET_BITMAP_CLIPPING);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetBitmapData, DISPLAY,
                                 SET_BITMAP_DATA);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetBitmapData, DISPLAY,
                                 GET_BITMAP_DATA);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetPartialBitmapData, DISPLAY,
                                 SET_PARTIAL_BITMAP_DATA);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetBitmapSize, DISPLAY,
                                 GET_BITMAP_SIZE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetBitmapID, DISPLAY,
                                 GET_BITMAP_ID);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_TransformBitmap, DISPLAY,
                                 TRANSFORM_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_TransformText, DISPLAY,
                                 TRANSFORM_TEXT);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DrawLine, DISPLAY, DRAW_LINE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DrawPolyline, DISPLAY,
                                 DRAW_POLYLINE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DrawPolygon, DISPLAY,
                                 DRAW_POLYGON);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DrawCircle, DISPLAY,
                                 DRAW_CIRCLE);
...
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_HasShaderSupport, DISPLAY,
                                 HAS_SHADER_SUPPORT);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_CreateShader, DISPLAY,
                                 CREATE_SHADER);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_DeleteShader, DISPLAY,
                                 DELETE_SHADER);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_StartShader, DISPLAY,
                                 START_SHADER);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_StopShader, DISPLAY,
                                 STOP_SHADER);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetParameterID, DISPLAY,
                                 GET_PARAMETER_ID);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetShaderBitmap, DISPLAY,
                                 SET_SHADER_BITMAP);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetShaderFloat, DISPLAY,
                                 SET_SHADER_FLOAT);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetShaderVector, DISPLAY,
                                 SET_SHADER_VECTOR);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetShaderID, DISPLAY,
                                 GET_SHADER_ID);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_EnableVSync, DISPLAY,
                                 ENABLE_VSYNC);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_IsVSyncEnabled, DISPLAY,
                                 IS_VSYNC_ENABLED);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetFullScreen, DISPLAY,
                                 SET_FULL_SCREEN);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_IsFullScreen, DISPLAY,
                                 IS_FULL_SCREEN);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetVideoModeCount, DISPLAY,
                                 GET_VIDEO_MODE_COUNT);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_GetVideoMode, DISPLAY,
                                 GET_VIDEO_MODE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_SetVideoMode, DISPLAY,
                                 SET_VIDEO_MODE);
orxPLUGIN_USER_CORE_FUNCTION_ADD(orxDisplay_GLFW_IsVideoModeAvailable, DISPLAY,
                                 IS_VIDEO_MODE_AVAILABLE);
